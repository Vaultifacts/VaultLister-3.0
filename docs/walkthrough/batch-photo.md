...
| P4-photo-2 | Batch Photo / Photo Tools | `image_bank` table missing `cloudinary_public_id` column — AI edits failed with SQL error; images uploaded before fix also caused ENOENT on Railway ephemeral FS | 2026-04-26 code audit | FIXED ✅ — commit 30b0afc9: migration 028 adds column, imageBank.js POST /upload now dual-writes to Cloudinary (fire-and-forget) so new images get `cloudinary_public_id` immediately |
