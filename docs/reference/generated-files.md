...
| `docs/OPEN_ITEMS.md` | `docs/open-items/items.json`, current `docs/walkthrough/` status tables, `docs/reference/deep-dive-backlog.md`, live GitHub issue query, explicit checklist backlogs, and TODO/FIXME source scan | `scripts/generate-open-items.mjs` | Never edit directly — update the relevant source document or `docs/open-items/items.json`, then regenerate | `bun run open-items && bun run open-items:check` |
