...
- **Outgoing webhook migration fixture CI isolation:** committed in `cf4657e2` — changed `src/tests/service-outgoingWebhooks-unit.test.js` to read `pg-schema.sql` with `Bun.file(new URL(..., import.meta.url))` instead of `node:fs`/`node:path`, after Dependabot PR #524 / issue #527 and a local focused reproduction showed `browser-profiles.test.js` global `mock.module('fs')` contamination causing the migration fixture to read browser-profile JSON. Verified by the polluted pair reporting 116 pass/0 fail, the outgoing webhook unit file reporting 87 pass/0 fail, `node --check`, `bun run lint:syntax`, `bun run lint`, and `git diff --check`.
- **Infra source assertion CI isolation:** committed in `a749e78f` — changed `src/tests/infra-backup-restore.test.js` to read repo files with `Bun.file(new URL(..., import.meta.url))` instead of `node:fs`/`node:path`, after GitHub CI run `25303074489` showed the new Railway verifier assertions reading `""` under existing full-suite `mock.module('fs')` contamination. Verified by focused infra test with 16 pass/0 fail, pairing the infra test with `browser-profiles.test.js` and `bot-abort-cancellation.test.js` fs-mock suites, `node --check`, `bun run lint:syntax`, and `git diff --check`.
- **Railway wait-for-CI deploy verifier classification:** committed in `392e6df5` — `.github/workflows/deploy.yml` no longer hides the Railway commit verifier behind `continue-on-error`; it classifies the known Railway `WAITING` + expected-commit state as `pending`, records `passed`/`pending`/`skipped`/`failed` to step outputs, and makes the Cloudflare challenge warning report that real status instead of claiming commit verification passed. Verified by the official Railway GitHub autodeploy docs for Wait for CI behavior, GitHub Deploy run `25301965006` logs, focused infra test with 16 pass/0 fail, PyYAML workflow parse, extracted Bash `bash -n`, `node --check`, `bun run lint:syntax`, and `git diff --check`.
- **Protected-prefix test CI isolation:** committed in `8d2cbae3` — changed `src/tests/server-protected-prefixes.test.js` from `node:fs`/`node:path` imports to `Bun.file(new URL(..., import.meta.url))` after GitHub CI run `25301658598` failed only because the new test was exposed to existing global `mock.module('fs')` contamination and read `"[]"` in the full `bun test src/tests/` process. Verified by focused protected-prefix test, protected-prefix test paired with both `browser-profiles.test.js` and `bot-abort-cancellation.test.js` fs-mock suites, `node --check`, `bun run lint:syntax`, and `git diff --check`.
- **Goals/budget API auth-prefix repair:** committed in `ef53b29c` — added `/api/goals` and `/api/budget` to `server.js` `protectedPrefixes` so authenticated requests populate `ctx.user` before reaching the existing `goalsRouter` and `budgetRouter`. Without this, the mounted routers were reachable but always saw `user = null` and returned 401 even with a bearer token. Verified by the new `server-protected-prefixes.test.js` reporting 1 pass / 0 fail, independent `bun -e` protected-prefix assertion, `node --check` for server and test files, `bun run lint:syntax`, and `git diff --check`.
- **Lighthouse deploy-race fix and #516 closure:** committed in `82bb471a` — changed the Lighthouse Score Regression workflow from direct `push` execution to `workflow_run` after successful Deploy completion, preventing audits from racing Railway before the pushed container is live. Root cause was verified from run #421's artifact (`horizontal-2048.svg`, CLS 0.87) versus fresh current-production runs after deploy. Verified by `npx yaml-lint .github/workflows/lighthouse.yml`, `git diff --check -- .github/workflows/lighthouse.yml`, local Lighthouse JSON against current production (Performance 99/100, CLS 0), manual GitHub Lighthouse run #422 (98/100 Performance), deploy-triggered Lighthouse run (100/100 Performance), and issue #516 closure.
- **SonarCloud previous-version baseline + open-items release refresh:** committed in `ba95b3ad` — changed SonarCloud new-code configuration from a pre-analysis specific `package.json` version to the documented `previous_version` mode after the 1.0.1 release failed with `Invalid new code period '1.0.1'`, and refreshed `docs/OPEN_ITEMS.md` through `c37fa662`. Verified by `bun run open-items:check`, independent competitor reconciliation showing 862 expected rows, 862 report rows, 0 unparsed open marker rows, YAML parse of `.github/workflows/sonarcloud.yml`, and `git diff --check`.
- **SonarCloud baseline API auth lint repair:** committed in `bb018c75` — changed the SonarCloud settings API calls from `curl -u "${SONAR_TOKEN}:"` to the documented bearer-token header form after gitleaks flagged the Basic-auth curl pattern as `curl-auth-user`. Verified by the gitleaks job log, official SonarCloud Web API authentication docs, `npx yaml-lint .github/workflows/sonarcloud.yml`, `git diff --check`, and grep confirming the flagged curl-auth pattern is gone.
- **SonarCloud specific-version baseline enforcement:** committed in `d8c01949` — after `1875b35a`, SonarCloud showed `projectVersion=1.6.1` but the quality-gate period still used `previous_version` from 2026-03-22, so the workflow now sets `sonar.leak.period=1.6.1` and `sonar.leak.period.type=version` via the documented SonarCloud settings API before analysis. Verified by live SonarCloud project-analysis/quality-gate APIs, official SonarCloud new-code-definition docs, `npx yaml-lint .github/workflows/sonarcloud.yml`, and `git diff --check`.
- **Public HTML lint repair after concurrent formatting pass:** committed in `7b6a3e58` — restored uppercase `<!DOCTYPE html>` and html-validate-compatible void elements in the seven pages covered by `lint:html` after concurrent master commits reintroduced lowercase doctypes and XHTML-style self-closing tags. Verified by `bun run lint`, `bun run lint:html`, `git diff --check`, and a grep check for lowercase doctypes/self-closing lint-target void tags.
