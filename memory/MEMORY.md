...
- **SonarCloud previous-version baseline + open-items release refresh:** committed in `7d40e5e1` — changed SonarCloud new-code configuration from a pre-analysis specific `package.json` version to the documented `previous_version` mode after the 1.0.1 release failed with `Invalid new code period '1.0.1'`, and refreshed `docs/OPEN_ITEMS.md` against `d94c98f0`. Verified by `bun run open-items:check`, independent competitor reconciliation showing 862 expected rows, 862 report rows, 0 unparsed open marker rows, YAML parse of `.github/workflows/sonarcloud.yml`, and `git diff --check`.
