...
export const LOW_CONFIDENCE_SIGNALS = new Set([SIGNAL_TYPES.LISTING_INVISIBLE, SIGNAL_TYPES.ENGAGEMENT_DROP]);

export const MEDIUM_CONFIDENCE_SIGNALS = new Set([SIGNAL_TYPES.API_LATENCY_ANOMALY]);
...
export function isListingInvisibleSignal({ inVaultlisterDb, notInMarketplaceSearchTwiceIn2h, notSold, ageHours }) {
    return (
...
export function isEngagementDropSignal({ rolling7dayPerListing, rolling30dayBaseline, listingCountInPeriod }) {
...
export function isApiLatencyAnomalySignal({ rolling5minMedianMs, rolling30dayMedianMs, requestCountInPeriod }) {
