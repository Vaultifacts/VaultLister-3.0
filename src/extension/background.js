...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === 'import_listing') {
        handleImportListing(sender.tab).then(sendResponse).catch(err => {
...
function scrapeListing() {
    ...
    if (host === 'poshmark.com' || host.endsWith('.poshmark.com') || host.includes('poshmark')) {
        ...
        result.price = priceEl ? parseFloat(priceEl.textContent.replace(/[^0-9.]/g, '')) : null;
        result.images = [...document.querySelectorAll('.listing-page-image-carousel img, [data-carousel] img')].map(i => i.src).filter(Boolean);
    }
    ...
    if (!result.description) {
        result.description = document.querySelector('meta[property="og:description"], meta[name="description"]')?.content || '';
    }
