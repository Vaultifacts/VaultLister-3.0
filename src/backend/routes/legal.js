...
const ALLOWED_AUDIT_TABLES = new Set([
    ...
    'checklists', 'automation_rules', 'calendar_events',
    'image_bank', 'suppliers', 'feedback_submissions', 'financial_transactions'
]);
...
async function acceptTos(user, body) {
  try {
    ...
    const existing = await query.get(
      'SELECT id FROM tos_acceptances WHERE user_id = ? AND tos_version_id = ?',
      [user.id, tosVersionId]
    );
