    ...
    try {
        ...
        if (method === 'POST' && path === '/scrape') {
            ...
            try {
                ...
                const product = await query.get(`SELECT * FROM scraped_products WHERE id = ?`, [productId]);
                ...
                logger.error('[Extension] error saving scraped product', user?.id, {
        ...
        if (method === 'POST' && path === '/price-tracking') {
            ...
            try {
                ...
                const tracking = await query.get(`SELECT * FROM price_tracking WHERE id = ? AND user_id = ?`, [
                    trackingId,
                    user.id,
                ]);
                ...
                logger.error('[Extension] error adding price tracking', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'GET' && path === '/price-tracking') {
            ...
            try {
                ...
                logger.error('[Extension] error fetching price tracking', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'PATCH' && path.startsWith('/price-tracking/') && path.split('/').length === 3) {
            ...
            try {
                ...
                await query.run(`UPDATE price_tracking SET ${updates.join(', ')} WHERE id = ? AND user_id = ?`, params);
                ...
                logger.error('[Extension] error updating price tracking', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'DELETE' && path.startsWith('/price-tracking/') && path.split('/').length === 3) {
            ...
            try {
                await query.run(`DELETE FROM price_tracking WHERE id = ? AND user_id = ?`, [trackingId, user.id]);
                ...
                logger.error('[Extension] error deleting price tracking', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'POST' && path === '/quick-add') {
            ...
            try {
                ...
                logger.error('[Extension] error quick adding item', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'GET' && path.startsWith('/autofill/') && path.split('/').length === 3) {
            ...
            try {
                const item = await query.get(`SELECT * FROM inventory WHERE id = ? AND user_id = ?`, [itemId, user.id]);
                ...
                logger.error('[Extension] error getting autofill data', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'GET' && path === '/scraped') {
            ...
            `;
                ...
                logger.error('[Extension] error fetching scraped products', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'DELETE' && path.startsWith('/scraped/') && path.split('/').length === 3) {
            ...
            try {
                await query.run(`DELETE FROM scraped_products WHERE id = ? AND user_id = ?`, [productId, user.id]);
                ...
                logger.error('[Extension] error deleting scraped product', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'POST' && path === '/sync') {
            ...
            try {
                ...
                const item = await query.get(`SELECT * FROM extension_sync_queue WHERE id = ?`, [syncId]);
                ...
                logger.error('[Extension] error adding to sync queue', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'GET' && path === '/sync') {
            ...
            `;
                ...
                logger.error('[Extension] error fetching sync queue', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'POST' && path.match(/^\/sync\/[a-zA-Z0-9_-]+\/process$/)) {
            ...
            try {
                ...
                logger.error('[Extension] error processing sync item', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
        ...
        if (method === 'POST' && path.match(/^\/sync\/[a-zA-Z0-9_-]+\/result$/)) {
            ...
            try {
                // Verify ownership
                const syncItem = await query.get(`SELECT * FROM extension_sync_queue WHERE id = $1 AND user_id = $2`, [
                    syncId,
                    user.id,
                ]);
                ...
                logger.error('[Extension] error reporting sync result', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
                return {
                    status: 500,
            ...
            try {
                ...
                if (!shop || !shop.platform_username) {
                    return {
                        status: 422,
                        data: {
                            error: 'No connected Poshmark account found. Connect your Poshmark account in My Shops first.',
                        },
                    };
                }
                ...
                logger.error('[Extension] error queuing share-closet job', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
        ...
        if (method === 'POST' && path === '/report-sale') {
            const { platform_order_id, platform = 'poshmark', title, sale_price, buyer_username, listing_url } = body;
            ...
            try {
                ...
                logger.error('[Extension] error recording sale', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
            ...
            try {
                ...
                if (!owned) {
                    return {
                        status: 422,
                        data: {
                            error: 'No matching listing found for this URL in your account. Only listings cross-listed through VaultLister can use OTL.',
                        },
                    };
                }
                ...
                return {
                    status: 201,
                    data: { success: true, syncId, listing_url, offer_price: parsedOffer, shipping_discount },
                };
            } catch (error) {
                logger.error('[Extension] error queuing OTL job', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
