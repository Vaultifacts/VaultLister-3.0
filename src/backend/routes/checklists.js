...
export async function checklistsRouter(ctx) {
    try {
        ...
        if (method === 'DELETE' && path.match(/^\/shares\/[a-f0-9-]+$/)) {
            ...
            try {
                const result = await query.run(`DELETE FROM checklist_shares WHERE id = ? AND user_id = ?`, [
                    shareId,
                    user.id,
                ]);
                if (result.changes === 0) {
        ...
        if (method === 'DELETE' && path.match(/^\/items\/[a-f0-9-]+$/)) {
            try {
                ...
                await query.run(`DELETE FROM checklist_items WHERE id = ? AND user_id = ?`, [itemId, user.id]);
        ...
        logger.error('[Checklists] Unhandled route error', {
            path: ctx.path,
            method: ctx.method,
            error: error.message,
        });
