...
export async function automationsRouter(ctx) {
    try {
        ...
        if (method === 'GET' && (path === '/' || path === '')) {
            ...
            rules.forEach((rule) => {
                try {
                    ...
                    logger.warn('[Automations] failed to parse automation conditions', {
                ...
                try {
                    ...
                    logger.warn('[Automations] failed to parse automation actions', {
                        detail: error?.message || 'Unknown error',
                    });
                    rule.actions = {};
                }
        ...
        if (method === 'GET' && path === '/logs') {
            ...
            const total =
                Number(
                    (await query.get('SELECT COUNT(*) as count FROM automation_logs WHERE user_id = ?', [user.id]))
                        ?.count,
                ) || 0;
        ...
        if (method === 'GET' && path.match(/^\/run\/[a-f0-9-]+\/logs$/)) {
            ...
            try {
                const run = await query.get('SELECT * FROM automation_runs WHERE id = ? AND user_id = ?', [
                    runId,
                    user.id,
                ]);
            ...
            `,
                ...
                logger.error('[Automations] failed to fetch run logs', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
        ...
        if (method === 'GET' && path === '/history') {
            ...
            if (
                !Number.isFinite(parsedHistLimit) ||
                !Number.isFinite(parsedHistOffset) ||
                parsedHistLimit < 0 ||
                parsedHistOffset < 0
            ) {
                return { status: 400, data: { error: 'limit and offset must be valid non-negative numbers' } };
            ...
            try {
                ...
                const total =
                    Number(
                        (await query.get('SELECT COUNT(*) as count FROM automation_runs WHERE user_id = ?', [user.id]))
                            ?.count,
                    ) || 0;
            ...
            `,
                ...
                logger.error('[Automations] failed to fetch automation history', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
        ...
        if (method === 'GET' && path === '/history/export') {
            try {
                ...
                logger.error('[Automations] failed to export history', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
        ...
        if (method === 'DELETE' && path === '/history') {
            try {
                ...
                logger.error('[Automations] failed to clear history', user?.id, {
                    detail: error?.message || 'Unknown error',
                });
        ...
        if (method === 'POST' && (path === '/' || path === '')) {
            ...
            if (conditionsStr.length > 50000)
                return { status: 400, data: { error: 'Conditions JSON exceeds maximum size (50 KB)' } };
            if (actionsStr.length > 50000)
                return { status: 400, data: { error: 'Actions JSON exceeds maximum size (50 KB)' } };
        ...
        `,
            ...
            try {
                ...
                logger.error('[Automations] error parsing rule conditions', user?.id, {
            ...
            try {
                ...
                logger.error('[Automations] error parsing rule actions', user?.id, {
                    detail: e?.message || 'Unknown error',
                });
                rule.actions = {};
            }
        ...
        if (method === 'PUT' && path.match(/^\/[a-f0-9-]+$/)) {
            ...
            const existing = await query.get('SELECT * FROM automation_rules WHERE id = ? AND user_id = ?', [
                id,
                user.id,
            ]);
            if (!existing) {
            ...
            try {
                const maxVer = await query.get(
                    'SELECT MAX(version) as v FROM automation_rule_versions WHERE rule_id = ?',
                    [id],
                );
                ...
                if (changes.length > 0) {
                    await query.run(
                        `INSERT INTO automation_rule_versions (id, rule_id, user_id, version, name, type, platform, schedule, conditions, actions, change_summary)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [
                            uuidv4(),
                            id,
        ...
        if (method === 'POST' && path.match(/^\/[a-f0-9-]+\/clone$/)) {
            ...
            const rule = await query.get('SELECT * FROM automation_rules WHERE id = ? AND user_id = ?', [
                ruleId,
                user.id,
            ]);
            ...
            const cloned = await query.get('SELECT * FROM automation_rules WHERE id = ? AND user_id = ?', [
                newId,
                user.id,
            ]);
        ...
        `,
            ...
            if (recentRun) {
                return {
                    status: 429,
                    data: { error: 'Please wait at least 60 seconds between manual runs of the same rule' },
                };
            }
        ...
        if (method === 'POST' && path.match(/^\/[a-f0-9-]+\/toggle$/)) {
            ...
            await query.run(
                'UPDATE automation_rules SET is_enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?',
        ...
        `,
            ...
            try {
                ...
                logger.error('[Automations] error parsing rule conditions from preset', user?.id, {
                    detail: e?.message || 'Unknown error',
                });
                rule.conditions = {};
            }
            try {
                ...
                logger.error('[Automations] error parsing rule actions from preset', user?.id, {
                    detail: e?.message || 'Unknown error',
                });
                rule.actions = {};
            }
        ...
        if (method === 'GET' && path === '/stats') {
            const stats = {
                totalRules:
                    Number(
                        (await query.get('SELECT COUNT(*) as count FROM automation_rules WHERE user_id = ?', [user.id]))
                            ?.count,
                    ) || 0,
                activeRules:
                    Number(
                        (
                            await query.get(
                                'SELECT COUNT(*) as count FROM automation_rules WHERE user_id = ? AND is_enabled = TRUE',
                                [user.id],
                            )
                        )?.count,
                    ) || 0,
                ...
                successfulRuns:
                    Number(
                        (
                            await query.get(
                                'SELECT COUNT(*) as count FROM automation_logs WHERE user_id = ? AND status = ?',
                                [user.id, 'success'],
                            )
                        )?.count,
                    ) || 0,
        ...
        if (method === 'POST' && path === '/experiments') {
            ...
            try {
                const baseRule = await query.get('SELECT * FROM automation_rules WHERE id = ? AND user_id = ?', [
                    baseRuleId,
                    user.id,
                ]);
            ...
            `,
                ...
                return {
                    status: 201,
                    data: { experiment: { id: experimentId, base_rule_id: baseRuleId, variant_rule_id: variantId } },
                };
            } catch (error) {
        ...
        if (method === 'PUT' && path.match(/^\/experiments\/[a-f0-9-]+$/)) {
            ...
            try {
                const exp = await query.get('SELECT * FROM automation_experiments WHERE id = ? AND user_id = ?', [
                    experimentId,
                    user.id,
                ]);
                ...
                `,
                    ...
                    await query.run(
                        'UPDATE automation_rules SET is_enabled = FALSE, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?',
                        [loserId, user.id],
                    );
                } else if (newStatus) {
                    await query.run(
                        'UPDATE automation_experiments SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
                        [newStatus, experimentId],
                    );
                }
        ...
        if (method === 'GET' && path === '/export') {
            try {
                const rules = await query.all(
                    'SELECT name, type, platform, schedule, conditions, actions FROM automation_rules WHERE user_id = ? ORDER BY name LIMIT 200',
                    [user.id],
                );
                ...
                return {
                    status: 200,
                    data: { rules: parsed, count: parsed.length, exported_at: new Date().toISOString() },
                };
            } catch (error) {
        ...
        if (method === 'POST' && path === '/import') {
            ...
            if (!Array.isArray(rules) || rules.length === 0)
                return { status: 400, data: { error: 'rules array required' } };
            ...
            try {
                ...
                for (const r of rules) {
                    ...
                    const existing = await query.get(
                        'SELECT id FROM automation_rules WHERE user_id = ? AND name = ? AND platform = ?',
                        [user.id, r.name, r.platform || 'all'],
                    );
                    if (existing) {
                        skipped++;
                        continue;
                    ...
                    await query.run(
                        `INSERT INTO automation_rules (id, user_id, name, type, platform, schedule, conditions, actions, is_enabled) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0)`,
                        [
                            uuidv4(),
                            user.id,
                            r.name,
        ...
        if (method === 'POST' && path === '/templates/share') {
            ...
            try {
                ...
                await query.run(
                    `INSERT INTO automation_templates (id, author_id, name, type, platform, schedule, conditions, actions, description, tags, is_public)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                    [
                        templateId,
                        user.id,
        ...
        if (method === 'POST' && path === '/templates/install') {
            ...
            try {
                const tpl = await query.get('SELECT * FROM automation_templates WHERE id = ? AND is_public = TRUE', [
                    templateId,
                ]);
                ...
                await query.run('INSERT INTO automation_template_installs (template_id, user_id) VALUES (?, ?)', [
                    templateId,
                    user.id,
                ]);
        ...
        if (method === 'POST' && path === '/reorder') {
            ...
            try {
                for (let i = 0; i < order.length; i++) {
                    await query.run('UPDATE automation_rules SET sort_order = ? WHERE id = ? AND user_id = ?', [
                        i,
                        order[i],
                        user.id,
                    ]);
        ...
        if (method === 'GET' && path.match(/^\/[a-f0-9-]+\/versions$/)) {
            const ruleId = path.split('/')[1];
            try {
                const rule = await query.get('SELECT id FROM automation_rules WHERE id = ? AND user_id = ?', [
                    ruleId,
                    user.id,
                ]);
                if (!rule) return { status: 404, data: { error: 'Rule not found' } };
                const versions = await query.all(
                    'SELECT * FROM automation_rule_versions WHERE rule_id = ? ORDER BY version DESC LIMIT 50',
                    [ruleId],
                );
        ...
        if (method === 'POST' && path.match(/^\/[a-f0-9-]+\/rollback$/)) {
            ...
            try {
                ...
                if (!rule) return { status: 404, data: { error: 'Rule not found' } };
                const ver = await query.get('SELECT * FROM automation_rule_versions WHERE id = ? AND rule_id = ?', [
                    versionId,
                    ruleId,
                ]);
                ...
                await query.run(
                    `UPDATE automation_rules SET name = ?, platform = ?, schedule = ?, conditions = ?, actions = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        ...
        if (method === 'POST' && path === '/templates/import-url') {
            ...
            try {
                ...
                for (const tpl of rules) {
                    ...
                    const ruleId = uuidv4();
                    await query.run(
                        `INSERT INTO automation_rules (id, user_id, name, type, platform, schedule, conditions, actions, is_enabled, tags)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)`,
                        [
                            ruleId,
                            user.id,
        ...
        if (method === 'POST' && path === '/poshmark/sync') {
            ...
            try {
                ...
                return {
                    status: 500,
                    data: { error: 'Failed to queue sync task: ' + (error?.message || 'Unknown error') },
                };
            }
        }
        ...
        if (method === 'GET' && path === '/scheduler-status') {
            try {
                ...
                const pending = await query.get('SELECT COUNT(*) as count FROM task_queue WHERE status = ?', [
                    'pending',
                ]);
                const processing = await query.get('SELECT COUNT(*) as count FROM task_queue WHERE status = ?', [
                    'processing',
                ]);
                const failed = await query.get(
                    "SELECT COUNT(*) as count FROM task_queue WHERE status = 'failed' AND created_at > NOW() - INTERVAL '24 hours'",
                );
            ...
            `,
                ...
                const enabledRules = await query.get(
                    'SELECT COUNT(*) as count FROM automation_rules WHERE user_id = ? AND is_enabled = TRUE',
                    [user.id],
                );
        ...
        logger.error('[Automations] Unhandled route error', {
            path: ctx.path,
            method: ctx.method,
            error: error.message,
        });
