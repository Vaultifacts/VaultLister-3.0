...
export async function analyticsRouter(ctx) {
    try {
        ...
        if (method === 'GET' && (path === '/dashboard' || path === '/stats')) {
            ...
            `,
                          [periodOffset, periodOffset, periodOffset, user.id],
                      )) || {}
                    : (await query.get(
                          `
                SELECT
        ...
        if (method === 'GET' && path === '/sustainability') {
            ...
            `,
                ...
                const salesCount =
                    Number(
                        (await query.get('SELECT COUNT(*) as count FROM sales WHERE user_id = ?', [user.id]))?.count,
                    ) || 0;
                    ...
                    data: {
                        ...
                        methodology: {
                            waterSaved: {
                                ...
                                sourceUrl:
                                    'https://www.wrap.org.uk/resources/report/valuing-our-clothes-true-cost-how-we-design-use-and-dispose-our-clothes',
                                accuracyRange: '±30%',
                                confidenceNote: 'Estimates vary by garment type, material, and manufacturing region',
                            },
                            co2Saved: {
                                description:
                                    'Estimated kg of CO₂-equivalent emissions avoided by reselling vs. new production',
                                source: 'thredUP 2023 Resale Report, citing Quantis lifecycle assessment data',
                                sourceUrl: 'https://www.thredup.com/resale/',
                                accuracyRange: '±25%',
                                confidenceNote:
                                    'CO₂ savings depend on item category, transport distance, and end-of-life of displaced new item',
                            },
                            wastePrevented: {
                                ...
                                accuracyRange: '±20%',
                                confidenceNote:
                                    'Based on average garment weight per category; actual weight may differ',
                            },
                            equivalents: {
                                showers: {
        ...
        if (method === 'GET' && path === '/heatmap') {
            ...
            return {
                ...
                data: {
                    ...
                    peakTimes,
                    days: safeDays,
                    ...(engagementError && { partial: true, warning: 'Engagement data unavailable' }),
                },
            };
        }
        ...
        if (method === 'GET' && path === '/heatmap/listings') {
            ...
            return {
                ...
                data: {
                    ...
                    ...(listingEngagementError && { partial: true, warning: 'Engagement data unavailable' }),
                },
            };
        }
        ...
        if (method === 'GET' && path === '/heatmap/geography') {
            ...
            const geoData = Array.from(buckets.values()).sort((a, b) => b.revenue - a.revenue);
        ...
        if (method === 'GET' && path === '/custom-metrics') {
            try {
                const metrics = await query.all(
                    'SELECT id, name, metric_a, operation, metric_b, display_format, created_at FROM custom_metrics WHERE user_id = ? ORDER BY created_at DESC LIMIT 100',
                    [user.id],
                );
        ...
        if (method === 'DELETE' && path.match(/^\/custom-metrics\/[a-f0-9-]+$/)) {
            try {
                ...
                const result = await query.run('DELETE FROM custom_metrics WHERE id = ? AND user_id = ?', [
                    metricId,
                    user.id,
                ]);
                if (result.changes === 0) {
        ...
        if (method === 'GET' && path === '/digest-settings') {
            try {
                ...
                return {
                    status: 200,
                    data: { settings: settings || { frequency: 'weekly', email: '', is_active: false } },
                };
            } catch (err) {
        ...
        if (method === 'POST' && path === '/export') {
            ...
            switch (type) {
                case 'inventory':
                    data = await query.all('SELECT * FROM inventory WHERE user_id = ? AND status != ? LIMIT 10000', [
                        user.id,
                        'deleted',
                    ]);
                    break;
        ...
        if (method === 'GET' && path === '/forecast') {
            ...
            `,
                ...
                return {
                    status: 200,
                    data: { forecasts, overall: { totalActive, totalValue, restockAlerts, overstocked } },
                };
            } catch (error) {
        ...
        if (method === 'GET' && path === '/price-suggestions') {
            ...
            `,
                ...
                const rateMap = {};
                for (const s of sellThrough)
                    rateMap[s.category] = { sell_rate: s.sell_rate || 0, avg_days: s.avg_days || 0 };
            ...
            try {
                ...
                await query.transaction(async () => {
                    ...
                    for (const item of validItems) {
                        ...
                        await query.run(
                            'UPDATE inventory SET list_price = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?',
                            [item.suggested_price, item.id, user.id],
                        );

                        try {
                            await query.run(
                                `INSERT INTO price_history (id, inventory_id, user_id, old_price, new_price, change_reason)
                            VALUES (?, ?, ?, ?, ?, ?)`,
                                [
                                    uuidv4(),
                                    item.id,
