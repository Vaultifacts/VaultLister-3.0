...
const ALLOWED_COLUMNS = {
  inventory: ['id', 'user_id', 'title', 'name', 'sku', 'category', 'brand', 'size', 'color', 'condition',
              'status', 'quantity', 'cost_price', 'list_price', 'location', 'source', 'notes',
              'acquired_date', 'created_at', 'updated_at'],
  listings:  ['id', 'user_id', 'title', 'platform', 'status', 'price', 'views', 'likes',
  ...
  _any: ['id', 'user_id', 'name', 'title', 'sku', 'category', 'brand', 'status', 'quantity',
         'cost_price', 'list_price', 'sale_price', 'platform', 'platform_fee', 'shipping_cost',
         'net_profit', 'created_at', 'updated_at', 'acquired_date', 'views', 'likes',
         'source', 'condition', 'notes', 'location', 'size', 'color']
};
...
function validateCustomQuery(sql, userId) {
  ...
  for (const ref of allTableRefs) {
    const tableName = ref.replace(/^(FROM|JOIN)\s+["`\[]?/i, '').replace(/["`\]]?$/, '').toLowerCase();
  ...
  try {
    switch (type) {
      ...
      case 'profit_margin': {
        ...
        return { type: 'stat', value: margin, label: 'Profit Margin', unit: '%',
      ...
      case 'platform_fees': {
        ...
        return { type: 'stat', value: row?.total_fees || 0, label: 'Platform Fees',
...
function parseReport(row) {
  let config = {};
  try { config = JSON.parse(row.config || '{}'); } catch (_) { logger.warn('Failed to parse report config', { rowId: row.id }); }
  return {
    id: row.id,
    name: row.name,
  ...
  try {
    ...
    if (method === 'POST' && (path === '/' || path === '')) {
      ...
      const storedConfig = isLegacy
        ...
        : JSON.stringify({ description: description || null, widgets: widgets || [], date_range: date_range || '30d' });
    ...
    if (method === 'GET' && path.match(/^\/[^\/]+$/) && !path.startsWith('/templates') && !path.startsWith('/pnl') && !path.startsWith('/query') && !path.startsWith('/widgets') && !path.startsWith('/generate')) {
    ...
    if (method === 'PUT' && path.match(/^\/[^\/]+$/) && !path.startsWith('/templates') && !path.startsWith('/pnl') && !path.startsWith('/query') && !path.startsWith('/widgets') && !path.startsWith('/generate')) {
      const reportId = path.slice(1);
      const { name, description, widgets, date_range, config } = body;

      const existingRow = await query.get(
        'SELECT * FROM saved_reports WHERE id = ? AND user_id = ?',
        [reportId, user.id]
      );
      ...
      if (description !== undefined || widgets !== undefined || date_range !== undefined || config !== undefined) {
        let existingConfig = {};
        try { existingConfig = JSON.parse(existingRow.config || '{}'); } catch (_) { logger.warn('Failed to parse existing report config'); }
      ...
      await query.run(
        `UPDATE saved_reports SET ${updates.join(', ')} WHERE id = ? AND user_id = ?`,
        params
      );
    ...
    if (method === 'DELETE' && path.match(/^\/[^\/]+$/) && !path.startsWith('/templates') && !path.startsWith('/pnl') && !path.startsWith('/query') && !path.startsWith('/widgets') && !path.startsWith('/generate')) {
      const reportId = path.slice(1);
    ...
    if (method === 'POST' && path.match(/^\/[^\/]+\/schedule$/)) {
      ...
      const existing = await query.get(
        'SELECT id FROM report_schedules WHERE report_id = ? AND user_id = ?',
        [reportId, user.id]
      );
