    ...
    try {
        ...
        if (method === 'POST' && path === '/keywords/analyze') {
         ...
         WHERE c.user_id = ?`,
            ...
            // Get user's listings
            const userListings = await query.all(`SELECT title FROM listings WHERE user_id = ?`, [user.id]);
            ...
            await query.run(`DELETE FROM competitor_keywords WHERE user_id = ?`, [user.id]);

            // Insert analyzed keywords with opportunity scores
            const insertPromises = [];
            for (const [keyword, data] of Object.entries(keywordMap)) {
                const avgPrice =
                    data.prices.length > 0 ? data.prices.reduce((a, b) => a + b, 0) / data.prices.length : null;
      ...
      `;
        ...
        logger.error('[CompetitorTracking] Competitor tracking router error', user?.id || null, {
