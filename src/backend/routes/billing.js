...
const PLANS = {
    free: {
        display_name: 'Free',
        price: 0,
        limits: { listings: 10, orders: 50, automations: 5, storage_mb: 100, api_calls: 1000 },
    },
    starter: {
        display_name: 'Starter',
        price: 9.99,
        limits: { listings: 100, orders: 500, automations: 20, storage_mb: 1000, api_calls: 10000 },
    },
    pro: {
        display_name: 'Pro',
        price: 24.99,
        limits: { listings: 500, orders: 2500, automations: 100, storage_mb: 5000, api_calls: 50000 },
    },
    business: {
        display_name: 'Business',
        price: 49.99,
        limits: { listings: -1, orders: -1, automations: -1, storage_mb: 25000, api_calls: 250000 },
    },
};
    ...
    try {
        ...
        if (method === 'POST' && path === '/usage/refresh') {
            ...
            const storageEstimate = Math.round(
                listingsCount.count * 0.5 + ordersCount.count * 0.1 + inventoryCount.count * 0.2,
            );
            ...
            for (const m of metrics) {
                await query.run(
                    ...
                    ON CONFLICT(user_id, metric, period_start) DO UPDATE SET
                        ...
                        updated_at = NOW()
                `,
                    [nanoid(), user.id, m.metric, m.value, m.limit, periodStart, periodEnd, m.value, m.limit],
                );
            }
        ...
        if (method === 'POST' && path === '/checkout') {
            ...
            if (!planId || !STRIPE_PRICE_IDS[planId]) {
                return {
                    status: 400,
                    data: { error: `Invalid planId. Paid plans: ${Object.keys(STRIPE_PRICE_IDS).join(', ')}` },
                };
            }
        ...
        if (method === 'POST' && path === '/cancel') {
            ...
            return {
                status: 200,
                data: { message: 'Subscription cancelled. You have been downgraded to the Free plan.' },
            };
        }
