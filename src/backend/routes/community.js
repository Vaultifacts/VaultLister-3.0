...
export async function communityRouter(ctx) {
    try {
        ...
        if (method === 'POST' && path.match(/^\/posts\/[^/]+\/replies$/)) {
            ...
            try {
                // Verify post exists
                const post = await query.get(`SELECT id FROM community_posts WHERE id = ?`, [postId]);
            ...
            try {
                ...
                if (existing) {
                    if (existing.reaction_type === reaction_type) {
                        ...
                        await query.run(`DELETE FROM community_reactions WHERE id = ? AND user_id = ?`, [
                            existing.id,
                            user.id,
                        ]);
        ...
        if (method === 'PATCH' && path.match(/^\/replies\/[a-f0-9-]+$/)) {
            ...
            try {
                // Verify reply exists
                const reply = await query.get(`SELECT id, user_id FROM community_replies WHERE id = ?`, [replyId]);
        ...
        if (method === 'DELETE' && path.match(/^\/posts\/[a-f0-9-]+$/)) {
            ...
            try {
                // Verify ownership
                const post = await query.get(`SELECT id FROM community_posts WHERE id = ? AND user_id = ?`, [
                    postId,
                    user.id,
                ]);
                ...
                await query.run(`UPDATE community_posts SET is_hidden = TRUE WHERE id = ? AND user_id = ?`, [
                    postId,
                    user.id,
                ]);
