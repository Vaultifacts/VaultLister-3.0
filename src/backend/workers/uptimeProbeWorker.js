...
async function writeHeartbeat() {
    try {
        ...
        logger.warn(`[UptimeProbeWorker] Heartbeat write failed (Redis may be unavailable): ${err?.message || 'unknown'}`);
    }
}
...
async function probeMarketOnce(platform) {
    ...
    try {
        ...
        return { up: false, latencyMs: Date.now() - start, error: (err && err.name === 'AbortError') ? 'timeout' : (err?.message || 'unknown').slice(0, 500) };
    } finally {
        clearTimeout(timer);
    }
}
...
async function probeVL(platform) {
    ...
    try {
        ...
        return { up: false, latencyMs: Date.now() - start, error: ('import failed: ' + (err?.message || 'unknown')).slice(0, 500) };
    }
    ...
    if (typeof mod.healthCheck === 'function') {
        ...
        const timer = setTimeout(() => { timeoutHit = true; controller.abort(); }, PROBE_TIMEOUT_MS);
        try {
            ...
            return { up: false, latencyMs: Date.now() - start, error: ('healthCheck threw: ' + (err?.message || 'unknown')).slice(0, 500) };
        } finally {
            clearTimeout(timer);
        }
    }
...
async function runProbes() {
    ...
    try {
        ...
        logger.info(`[UptimeProbeWorker] Probed ${PLATFORMS.length} platforms (${PLATFORMS.length * 2} samples) in ${Date.now() - lastRun}ms`);
