...
const { mockOAuthRouter } =
    process.env.NODE_ENV !== 'production' ? await import('./routes/mock-oauth.js') : { mockOAuthRouter: null };
import { websocketService } from './services/websocket.js';
import { stopRateLimitDashboard } from './routes/rateLimitDashboard.js';
...
const BUILD_HASH = (() => {
    try {
        const raw =
            Bun.spawnSync(['git', 'rev-parse', '--short', 'HEAD'], { cwd: ROOT_DIR }).stdout.toString().trim() ||
...
const SAFE_CHUNK_RE =
    /^\/[a-zA-Z0-9_\-./]+\.(js|css|json|html|svg|png|jpg|jpeg|gif|ico|woff|woff2|ttf|webp|webmanifest|xml|txt|yaml|yml)$/;
...
async function main() {
    ...
    if (IS_PROD && process.env.OAUTH_MODE !== 'real') {
        logger.warn(
            '[SECURITY] OAUTH_MODE is not set to "real" in production — all marketplace connections will use mock tokens. Set OAUTH_MODE=real in environment variables.',
        );
    }
    ...
    function withRequestTimeout(handlerPromise, timeoutMs) {
        ...
        return new Promise((resolve) => {
            const timer = setTimeout(() => {
                resolve(
                    new Response(JSON.stringify({ error: 'Request Timeout', status: 408 }), {
                        status: 408,
...
<div class="card">
  ...
  <p>VaultLister is temporarily down for scheduled maintenance.<br>Check back shortly — we appreciate your patience.</p>
            ...
            if (
                ...
                (pathname.startsWith('/mock-oauth/') || pathname.startsWith('/api/mock-oauth/'))
            ) {
                const context = {
                    method,
                            ...
                            Sentry.startSpan(
                                ...
                                async () => {
                                    ...
                                    const isPublicMonitoring =
                                        effectivePath === '/api/monitoring/rum' && method === 'POST';
                                    ...
                                    if (body && body.error === 'Request body too large') {
                                        ...
                                        return new Response(
                                            JSON.stringify({ error: 'Request body too large', maxSize: `${maxMB}MB` }),
                                            {
                                                status: 413,
                                                headers: { 'Content-Type': 'application/json', ...dynamicCorsHeaders },
                                            },
                                            ...
                                            try {
                                                ...
                                                if (result?.isStream) {
                                                    ...
                                                    if (context.csrfToken)
                                                        streamHeaders['X-CSRF-Token'] = context.csrfToken;
                                                    return new Response(result.body, {
                                                        status: 200,
