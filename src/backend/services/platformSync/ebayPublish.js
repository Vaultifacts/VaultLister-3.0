...
async function ensureInventoryLocation(token) {
    ...
    const createResult = await ebayRequest(
        'POST',
...
async function ensureMerchantPolicies(token, marketplaceId = 'EBAY_US') {
    ...
    if (!optInResult.ok && optInResult.status !== 409) {
        ...
        logger.warn('[eBay Publish] Could not opt into Selling Policy Management', { status: optInResult.status, msg: errMsg });
    }
...
export async function publishListingToEbay(shop, listing, inventory) {
    ...
    logger.info('[eBay Publish] Resolved condition', { input: inventory.condition, resolved: condition, categoryId });
    ...
    try {
        ...
        logger.warn('[eBay Publish] Could not fetch required aspects, using available fields only', { categoryId, err: e.message });
    }
    ...
    if (offerResult.ok) {
        ...
        const updateResult = await ebayRequest('PUT', `/sell/inventory/v1/offer/${offerId}`, accessToken, offerPayload);
