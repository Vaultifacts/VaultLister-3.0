...
async function resolveShopId(token) {
    ...
    if (!shopsResult.ok)
        throw new Error(`Etsy shops fetch failed (${shopsResult.status}): ${JSON.stringify(shopsResult.data)}`);
...
async function ensureShippingProfileId(token, shopId) {
    ...
    const createResult = await etsyRequest('POST', `/v3/application/shops/${shopId}/shipping-profiles`, token, {
