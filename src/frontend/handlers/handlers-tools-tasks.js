...
Object.assign(handlers, {
    ...
    showDailyReview: function () {
        ...
        // Calculate streak
        const streakDays = streakCounter.calculate(
            tasks
                .filter((t) => t.completed)
                .map((t) => getCompletedAt(t))
                .filter(Boolean),
        );
    ...
    renderSizeComparison: function () {
                    ...
                    .map(
                        (row, i) =>
                            ...
                            row
                                .map(
                                    (c, j) =>
                                        `<td style="border: 1px solid var(--gray-200); padding: 4px 6px; ${j === 0 ? 'font-weight: 600;' : ''}">${c}</td>`,
                                )
                                .join('') +
                            '</tr>',
                    )
        ...
        container.innerHTML = sanitizeHTML(
            `<div>${renderMiniChart(selA.value)}</div><div>${renderMiniChart(selB.value)}</div>`,
    ...
    _getSizeData: function (tabId) {
        ...
        const data = {
            'women-clothing': {
                ...
                rows: [
                    ['XS', '0-2', '6-8', '32-34', '31-32', '23-24', '33-34'],
                    ['S', '4-6', '8-10', '36-38', '33-34', '25-26', '35-36'],
                    ['M', '8-10', '12-14', '40-42', '35-37', '27-29', '37-39'],
                    ['L', '12-14', '16-18', '44-46', '38-41', '30-33', '40-43'],
                    ['XL', '16-18', '20-22', '48-50', '42-45', '34-37', '44-47'],
                    ['XXL', '20-22', '24-26', '52-54', '46-49', '38-41', '48-51'],
                ],
            },
            'men-clothing': {
                ...
                rows: [
                    ['XS', '32-34', '42-44', '32-34', '26-28', '13-13.5'],
                    ['S', '35-37', '44-46', '35-37', '29-31', '14-14.5'],
                    ['M', '38-40', '48-50', '38-40', '32-34', '15-15.5'],
                    ['L', '41-43', '52-54', '41-43', '35-37', '16-16.5'],
                    ['XL', '44-46', '56-58', '44-46', '38-40', '17-17.5'],
                    ['XXL', '47-49', '60-62', '47-49', '41-44', '18-18.5'],
                ],
            },
            'women-shoes': {
                title: "Women's Shoes",
            ...
            'kids-clothing': {
                title: "Kids' Clothing",
                headers: ['Age', 'US Size', 'UK Size', 'EU Size', 'Height (in)'],
                rows: [
                    ['2T', '2T', '18-24m', '86-92', '33-35'],
                    ['3T', '3T', '2-3', '92-98', '35-38'],
                    ['4T', '4T', '3-4', '98-104', '38-41'],
                    ['5', '5', '4-5', '104-110', '41-44'],
                    ['6', '6', '5-6', '110-116', '44-47'],
                    ['7', '7', '6-7', '116-122', '47-50'],
                ],
            },
            jeans: {
                title: 'Jeans & Pants',
                headers: ['US Size', 'Waist (in)', 'Inseam (in)', 'EU Size'],
                rows: [
                    ['26', '26', '30', '34'],
                    ['27', '27', '30', '35'],
                    ['28', '28', '30-32', '36'],
    ...
    calculateSizeRecommendation: async function () {
        ...
        try {
            ...
            sizeResultEl.innerHTML = sanitizeHTML(`
                <div class="card" style="padding: 16px; background: var(--bg-success-subtle); border: 1px solid var(--border-success);">
                    ...
                    <div style="margin-top: 8px; font-size: 13px;">
                        ${Object.entries(data.size_conversions || {})
                            .map(([sys, sz]) => `<span class="badge" style="margin: 2px;">${sys}: ${sz}</span>`)
                            .join('')}
                    </div>
                </div>
            `);
            ...
            if (sizeResultEl)
                sizeResultEl.innerHTML = sanitizeHTML(
                    '<p style="color: var(--text-error);">Failed to get recommendation</p>',
                ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
            console.error(err);
        }
    },
    ...
    showBrandSizeGuide: function (brandKey) {
        const brands = {
            nike: {
                name: 'Nike',
                note: 'Nike runs true to size. Athletic fit may run slim. For shoes, Nike uses standard US sizing. Wide width available in select styles.',
            },
            adidas: {
                name: 'Adidas',
                note: 'Adidas runs slightly large. Consider sizing down 0.5 for shoes. EU sizing is standard. Clothing uses UK/EU system predominantly.',
            },
            levis: {
                name: "Levi's",
                note: 'Shrink-to-fit (STF) jeans: buy 1-2 sizes up in waist, 3" up in inseam. Pre-shrunk styles run true to size. Curvy fits run small in waist.',
            },
            zara: {
                name: 'Zara',
                note: "Zara runs small, especially in tops and dresses. Size up 1 for US sizing. EU sizing is more accurate. Men's runs more true to size.",
            },
            hm: {
                name: 'H&M',
                note: 'H&M runs true to size. Slim fit options may require sizing up. Divided line runs smaller. Premium quality line is more generous.',
            },
            gap: {
                name: 'Gap',
                note: 'Gap tends to run slightly large, especially in relaxed fits. Slim and skinny fits are more true to size. Baby/kids runs large.',
            },
            uniqlo: {
                name: 'Uniqlo',
                note: 'Uniqlo uses Japanese sizing. Size up 1-2 for US equivalent. Lengths are shorter (designed for Asian body types). XL in Uniqlo = M/L in US.',
            },
            'ralph-lauren': {
                name: 'Ralph Lauren',
                note: 'Runs true to size. Classic fit is roomier than slim fit. Custom fit is between the two. Purple label runs slim.',
            },
        };
    ...
    toggleChecklistItem: async function (itemId, completed, parentId = null) {
        try {
            if (parentId) {
                ...
                const updatedItems = items.map((item) => {
                    if (item.id === parentId) {
                        const subtasks = (item.subtasks || []).map((st) =>
                            st.id === itemId
                                ? { ...st, completed, last_completed_at: completed ? new Date().toISOString() : null }
                                : st,
                        );
                ...
                const updatedItems = items.map((item) =>
                    item.id === itemId
                        ? { ...item, completed, last_completed_at: completed ? new Date().toISOString() : null }
                        : item,
                );
                store.setState({ checklistItems: updatedItems });
                if (store.state.currentPage === 'checklist') {
    ...
    checkChecklistCelebration: function () {
        ...
        if (allCompleted && !store.state._checklistCelebrated) {
            ...
            overlay.innerHTML =
                sanitizeHTML('<div class="celebration-content">') + // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
    ...
    deleteChecklistItem: async function (itemId, parentId = null) {
        ...
        if (
            !(await modals.confirm(`Delete this ${isSubtask ? 'subtask' : 'task'}?`, {
                title: `Delete ${isSubtask ? 'Subtask' : 'Task'}`,
                confirmText: 'Delete',
                danger: true,
            }))
        )
    ...
    createTodoList: async function () {
        const name = await modals.prompt('Enter a name for the new list:', {
            title: 'Create List',
    ...
    deleteTodoList: async function (listId) {
        if (
            !(await modals.confirm('Delete this list and all its items?', {
                title: 'Delete List',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    navigateCalendarMonth: function (direction) {
        const currentMonth =
            store.state.calendarMonth !== undefined ? store.state.calendarMonth : new Date().getMonth();
        const currentYear =
            store.state.calendarYear !== undefined ? store.state.calendarYear : new Date().getFullYear();
            ...
            modals.show(`
                ...
                <div class="modal-body">
                    <div style="display: flex; flex-direction: column; gap: 12px;">
                        ${events
                            .map((event) => {
                                const parentEvent = event.depends_on
                                    ? allEvents.find((e) => e.id === event.depends_on)
                                    : null;
    ...
    deleteCalendarEvent: async function (eventId) {
        if (
            await modals.confirm('Are you sure you want to delete this event?', {
                title: 'Delete Event',
                confirmText: 'Delete',
                danger: true,
            })
    ...
    createFolder: async function () {
        const name = await modals.prompt('Enter a name for the new folder:', {
            title: 'Create Folder',
    ...
    deleteFolder: async function (folderId, folderName) {
        const confirmed = await modals.confirm(
            'Delete Folder',
            `Are you sure you want to delete the folder "${folderName}"? Images in this folder will be moved to All Images.`,
        );
        if (!confirmed) return;
    ...
    moveSelectedImages: async function () {
        ...
        const folderId = await modals.prompt('Select a folder to move images to:', {
            title: 'Move Images',
            selectOptions: folders.map((f) => ({ value: String(f.id), label: f.name })),
        });
        if (!folderId) return;
    ...
    tagSelectedImages: async function () {
        ...
        const tagsInput = await modals.prompt('Enter tags to add (comma-separated):', {
            title: 'Add Tags',
            placeholder: 'e.g. shoes, nike, vintage',
        });
        if (!tagsInput) return;
    ...
    deleteSelectedImages: async function () {
        ...
        if (
            !(await modals.confirm(`Delete ${selected.length} image(s)? This cannot be undone.`, {
                title: 'Delete Images',
                confirmText: 'Delete',
                danger: true,
            }))
        ...
        try {
            ...
            try {
                ...
                if (usageContainer) {
                    ...
                    if (items.length > 0) {
                        // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                        usageContainer.innerHTML = sanitizeHTML(
                            items
                                .map(
                                    (item) => `
                        ...
                        usageContainer.innerHTML = sanitizeHTML(
                            '<div class="image-usage-empty">Not used in any listings yet</div>',
                        ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                    }
                }
            } catch (err) {
                ...
                if (usageContainer) {
                    usageContainer.innerHTML = sanitizeHTML(
                        '<div class="image-usage-empty">Could not load usage data</div>',
                    ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                }
            }
        } catch (error) {
    ...
    deleteImage: async function (imageId) {
        if (
            !(await modals.confirm('Delete this image? This cannot be undone.', {
                title: 'Delete Image',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    deleteReceipt: async function (receiptId) {
        if (
            !(await modals.confirm('Delete this receipt? This cannot be undone.', {
    ...
    deleteBatchPhotoPreset: async function (presetId) {
        if (
            !(await modals.confirm('Delete this preset?', {
                title: 'Delete Preset',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    cancelBatchPhotoJob: async function (jobId) {
        if (
            !(await modals.confirm('Cancel this batch job?', {
                title: 'Cancel Job',
    ...
    deleteBatchPhotoJob: async function (jobId) {
        if (
            !(await modals.confirm('Delete this batch job? This cannot be undone.', {
                title: 'Delete Job',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    selectAllBatchImages: function () {
        ...
        const imagesToSelect = selectedFolder ? images.filter((img) => img.folder_id === selectedFolder) : images;
            ...
            if (form) {
                ...
                if (data.category) {
                    ...
                    const option = Array.from(categorySelect.options).find(
                        (o) => o.textContent.toLowerCase() === data.category.toLowerCase(),
                    );
    ...
    deleteWhatnotEvent: async function (eventId) {
        if (
            !(await modals.confirm('Delete this live event?', {
                title: 'Delete Event',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    filterEventItemSearch: function (query) {
        ...
        if (listEl) {
            listEl.innerHTML = sanitizeHTML(
                filtered.length === 0 // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
    ...
    submitShareChecklist: async function () {
        ...
        try {
            ...
            showToast(
                `Checklist shared with ${recipients.length} recipient${recipients.length > 1 ? 's' : ''}`,
                'success',
            );
            modals.close();
        } catch (err) {
    ...
    saveCustomMetric: async function () {
        ...
        try {
            const res = await api.post('/analytics/custom-metrics', {
                name,
                metric_a,
                operation,
                metric_b,
            ...
            metrics.push(
                res.metric || { id: Date.now().toString(), name, metric_a, operation, metric_b, display_format },
            );
    ...
    addChecklistAttachment: function (mode) {
        ...
        if (!input || !input.value.trim()) {
            toast.warning('Enter a file name or URL');
            return;
        }
