...
Object.assign(handlers, {
    ...
    renderSizeComparison: function() {
        ...
        container.innerHTML =sanitizeHTML( sanitizeHTML(`<div>${renderMiniChart(selA.value)}</div><div>${renderMiniChart(selB.value)}</div>`));
    },
    ...
    calculateSizeRecommendation: async function() {
        ...
        try {
            ...
            document.getElementById('size-recommendation-result').innerHTML =sanitizeHTML( sanitizeHTML('<p style="color: var(--text-error);">Failed to get recommendation</p>'));
            console.error(err);
        }
    },
    ...
    checkChecklistCelebration: function() {
        ...
        if (allCompleted && !store.state._checklistCelebrated) {
            ...
            overlay.innerHTML =sanitizeHTML( sanitizeHTML('<div class="celebration-content">' +
            ...
            try {
                ...
                if (usageContainer) {
                    ...
                    if (items.length > 0) {
                        ...
                        usageContainer.innerHTML =sanitizeHTML( sanitizeHTML('<div class="image-usage-empty">Not used in any listings yet</div>'));
                    }
                }
            } catch (err) {
                const usageContainer = document.getElementById('image-usage-list');
                if (usageContainer) {
                    usageContainer.innerHTML =sanitizeHTML( sanitizeHTML('<div class="image-usage-empty">Could not load usage data</div>'));
                }
            }
        } catch (error) {
    ...
    scanForDuplicates: async function() {
        ...
        try {
            ...
            listEl.innerHTML =sanitizeHTML( sanitizeHTML(`<p style="color: var(--error);">Error: ${error.message}</p>`));
        }
    },
    ...
    filterEventItemSearch: function(query) {
        ...
        if (listEl) {
            listEl.innerHTML =sanitizeHTML( sanitizeHTML(filtered.length === 0 ?
