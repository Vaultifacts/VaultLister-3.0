...
Object.assign(handlers, {
    ...
    saveTransaction: function(e) {
        ...
        const duplicate = allTransactions.find(t =>
            Math.abs(t.amount - amount) < 0.01 && t.date === date
        );
    ...
    exportTransactions: function(format) {
        ...
        if (txSearchQuery) {
            ...
            purchases = purchases.filter(p => (p.vendor || p.vendor_name || '').toLowerCase().includes(q) || (p.item || '').toLowerCase().includes(q));
            sales = sales.filter(s => (s.listing_title || s.inventory_title || '').toLowerCase().includes(q) || (s.platform || '').toLowerCase().includes(q));
        }
        ...
        if (txBuyerFilter) { const bq = txBuyerFilter.toLowerCase(); sales = sales.filter(s => (s.buyer_username || s.buyer_name || '').toLowerCase().includes(bq)); }
        if (txDateFilter !== 'all') {
            const now = new Date();
            let cutoff;
            ...
            else if (txDateFilter === 'today') { cutoff = new Date(); cutoff.setHours(0, 0, 0, 0); }
            if (cutoff) {
        ...
        if (txAmountMin > 0) { purchases = purchases.filter(p => (p.amount || p.total_amount || 0) >= txAmountMin); sales = sales.filter(s => (s.sale_price || 0) >= txAmountMin); }
    ...
    bulkCategorizePurchases: async function(category) {
        if (!category) return;
        const selected = store.state.selectedPurchases || [];
        if (selected.length === 0) { toast.error('No purchases selected'); return; }
    ...
    deleteRecurringTemplate: async function(templateId) {
        if (!await modals.confirm('Delete this recurring template?', { title: 'Delete Template', confirmText: 'Delete', danger: true })) return;
        try {
    ...
    exportOrdersCSV: function() {
        ...
        // CSV headers
        const headers = ['Date', 'Platform', 'Buyer', 'Item', 'Status', 'Price', 'Tracking', 'Carrier', 'Expected Arrival'];
    ...
    showFollowUpReminder: function(orderId) {
        ...
        const reminderTypes = [
            { id: 'review', label: 'Request Review', icon: 'star', message: 'Hi! Thanks for your purchase. If you love your item, a review would be greatly appreciated!' },
            { id: 'delivery', label: 'Check Delivery', icon: 'package', message: 'Hi! Just checking in to make sure your order arrived safely. Let me know if you have any questions!' },
            { id: 'bundle', label: 'Offer Bundle Deal', icon: 'gift', message: 'Hi! I noticed you might like some other items in my closet. Would you be interested in a bundle discount?' },
            { id: 'return', label: 'Return Customer', icon: 'heart', message: 'Hi! Thanks for being a great customer. Check out my new arrivals - I\'d love to ship to you again!' },
    ...
    _showDailySummaryImpl: function() {
        ...
        const newListings = listings.filter(l => new Date(l.listed_at || l.created_at).toDateString() === todayStr).length;
        const activeListings = listings.filter(l => l.status === 'active').length;

        const newOffers = offers.filter(o => new Date(o.created_at).toDateString() === todayStr);
        const pendingOffers = offers.filter(o => o.status === 'pending').length;

        const completedTasks = tasks.filter(t => t.completed && new Date(t.last_completed_at || t.completed_at).toDateString() === todayStr).length;
        ...
        const topSale = todaySales.length > 0 ? todaySales.reduce((max, s) => (s.sale_price > max.sale_price ? s : max), todaySales[0]) : null;
                ...
                <div class="order-timeline-section" style="background: var(--gray-50); border-radius: var(--radius-lg); padding: 16px; margin-bottom: 16px;">
                    ...
                    <div class="order-timeline">
                        ${[
                            ...
                            return '<div class="order-timeline-step ' + (isComplete ? 'complete' : '') + ' ' + (isCurrent ? 'current' : '') + '">' +
                                '<div class="order-timeline-dot">' + components.icon(step.icon, 14) + '</div>' +
                                (i < arr.length - 1 ? '<div class="order-timeline-line ' + (stepIdx < currentIdx ? 'complete' : '') + '"></div>' : '') +
                                '<div class="order-timeline-label">' + step.label + '</div>' +
                                '<div class="order-timeline-date">' + (step.date ? new Date(step.date).toLocaleDateString() : (isComplete && step.status === 'pending' ? new Date(order.created_at).toLocaleDateString() : '--')) + '</div>' +
                            '</div>';
                        }).join('')}
                    </div>
                </div>
    ...
    submitReturn: async function(e, orderId) {
        ...
        try {
            ...
            // Update local state
            const orders = (store.state.orders || []).map(o => o.id === orderId ? (response.order || { ...o, ...data, return_status: 'requested' }) : o);
    ...
    updateReturnStatus: async function(orderId, newStatus) {
        try {
            ...
            const orders = (store.state.orders || []).map(o => o.id === orderId ? (response.order || { ...o, return_status: newStatus }) : o);
            store.setState({ orders });
    ...
    updateOrderPriority: async function(orderId, priority) {
        try {
            ...
            // Update local state
            const orders = (store.state.orders || []).map(o => o.id === orderId ? { ...o, priority: priority, priority_note: note } : o);
            store.setState({ orders });
    ...
    executeSplitShipment: async function(orderId) {
        try {
            ...
            if (!splitEl) { toast.error('Split count element not found'); return; }
    ...
    startReconciliation: function() {
        modals.show('Bank Reconciliation', `
    ...
    deletePurchase: async function(id) {
        if (!await modals.confirm('Are you sure you want to delete this purchase? This will also remove associated cost layers.', { title: 'Delete Purchase', confirmText: 'Delete', danger: true })) {
            return;
        }
        try {
            await api.ensureCSRFToken();
    ...
    deleteCategorizationRule: async function(ruleId) {
        if (!await modals.confirm('Delete this categorization rule?', { title: 'Delete Rule', confirmText: 'Delete', danger: true })) return;
    ...
    showAddAccount: function() {
        ...
        const typeOptions = accountTypes.map(group =>
    ...
    generateCustomReport: function() {
        ...
        switch (reportType) {
            case 'sales-summary':
                ...
                data = (store.state.sales || []).filter(s => {
    ...
    saveAnalyticsSettings: function(event, tab) {
        ...
        // Get hidden tabs
        const hiddenTabs = [];
        const allTabs = ['graphs', 'performance', 'heatmaps', 'predictions', 'reports', 'ratio-analysis', 'profitability-analysis', 'product-analysis'];
    ...
    exportAnalyticsCSV: function() {
        ...
        // CSV headers
        const headers = ['Date', 'Item', 'Platform', 'Sale Price', 'Item Cost', 'Shipping Cost', 'Platform Fee', 'Net Profit', 'Status'];
        ...
        rows.push([]);  // Empty row
        rows.push(['TOTALS', '', '', totalRevenue.toFixed(2), totalCost.toFixed(2), totalShipping.toFixed(2), totalFees.toFixed(2), totalProfit.toFixed(2), '']);
