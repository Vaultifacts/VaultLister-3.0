...
Object.assign(handlers, {
    ...
    showMarketplaceRequirements: function (platform) {
        const guides = {
            poshmark: {
                name: 'Poshmark',
                photoReqs:
                    'Min 1, max 16 photos. Cover photo is most important. Square format (1000x1000px). No watermarks, text overlays, or borders.',
                titleTips: 'Max 80 chars. Include brand, size, color, and key details. Use title case.',
                descTips:
                    'Max 1500 chars. Describe condition, measurements, materials, and flaws. Bullet points recommended.',
                pricingNotes:
                    'Set with shipping in mind. Poshmark takes 20% fee ($2.95 on sales under $15). Offers and bundles are key.',
                shippingInfo:
                    'USPS Priority Mail only. Poshmark provides label. Upgraded shipping available for heavier items.',
            ...
            ebay: {
                name: 'eBay',
                photoReqs:
                    'Min 1, max 24 photos. First photo is gallery image. Min 500x500px, recommended 1600x1600px. No stock photos for used items.',
                titleTips: 'Max 80 chars. Front-load with keywords. Include brand, model, size, condition.',
                descTips:
                    'No character limit. Use HTML formatting. Include measurements, condition details, and return policy.',
    ...
    settingsSearch: function (query) {
        ...
        const searchMap = [
            {
                terms: ['name', 'email', 'display', 'timezone', 'avatar', 'profile', 'personal'],
                tab: 'account',
                label: 'Personal Information',
                icon: 'user',
            },
            {
                terms: ['security', 'password', '2fa', 'two-factor', 'login'],
                tab: 'account',
            ...
            {
                terms: ['notification', 'email alerts', 'push', 'quiet hours', 'digest', 'alert'],
                tab: 'notifications',
                label: 'Notifications',
                icon: 'bell',
            },
            ...
            {
                terms: ['affiliate', 'referral', 'commission', 'payout', 'partner'],
                tab: 'affiliate',
                label: 'Affiliate Program',
                icon: 'user',
            },
            {
                terms: ['shipping', 'sku', 'webhook', 'listing defaults', 'watermark', 'tools'],
                tab: 'tools',
                label: 'Customization',
                icon: 'tool',
            },
            {
                terms: ['billing', 'plan', 'subscription', 'payment', 'pricing', 'upgrade', 'invoice'],
                tab: 'plans-billing',
                label: 'Billing & Plans',
                icon: 'credit-card',
            },
        ...
        const matches = searchMap.filter(
            (item) =>
        ...
        if (matches.length === 0) {
            resultsEl.innerHTML = sanitizeHTML(
                '<div style="padding:12px 16px; color:var(--gray-500); font-size:13px;">No matching settings found</div>',
            ...
            <div class="modal-body">
                ...
                <form onsubmit="handlers.verify2FACode(event, 'authenticator')">
                    <div class="form-group">
                        ...
                        <inputinput id="hsa-enter-the-6-digit-code-from-your-app" type="text" class="form-input" name="code" maxlength="6" pattern="[0-9]{6}" placeholder="000000" style="text-align: center; font-size: 24px; letter-spacing: 8px;" required>
                    </div>
                    <div class="flex justify-end gap-2 mt-4">
            ...
            <div class="modal-body">
                ...
                <form onsubmit="handlers.sendSMS2FACode(event)">
                    <div class="form-group">
                        ...
                        <inputinput id="hsa-phone-number" type="tel" class="form-input" name="phone" placeholder="+1 (555) 123-4567" required>
                    </div>
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="button" class="btn btn-secondary" onclick="handlers.enable2FA()">Back</button>
    ...
    startToolTour: function (toolName) {
        const tours = {
            automations: [
                {
                    ...
                    description: 'Each card represents an automation. Toggle the switch to enable/disable.',
                },
                {
                    selector: '.btn-primary',
                    title: 'Create New',
                    description: 'Click here to create a custom automation with the wizard.',
                },
            ],
            checklist: [
                {
                ...
                },
                {
                    ...
                    title: 'Focus Timer',
                    description: 'Use the Pomodoro technique to stay productive.',
                },
            ],
            inventory: [
                {
                    selector: '.inventory-table, .inventory-grid',
                    title: 'Your Inventory',
                    description: 'All your items in one place. Click any item to edit.',
                },
                {
                    selector: '[onclick*="showAddItemModal"], [onclick*="addItem"]',
                    title: 'Add Items',
                    description: 'Add new inventory items manually or import from marketplaces.',
                },
                {
                    selector: '[onclick*="showFilterMenu"]',
                    title: 'Filters',
                    description: 'Filter by category, platform, status, price range, and more.',
                },
                {
                    selector: '[onclick*="exportInventoryCSV"]',
                    title: 'Export',
                    description: 'Export your inventory as CSV for spreadsheets or backup.',
                },
            ],
            analytics: [
                {
                    selector: '.analytics-hero, .page-header',
                    title: 'Analytics Overview',
                    description: 'Your sales performance at a glance — revenue, profit, and trends.',
                },
                {
                    selector: '[onclick*="switchAnalyticsTab"]',
                    title: 'Tab Navigation',
                    description: 'Switch between Sales, Performance, Reports, and more.',
                },
                {
                    selector: '[onclick*="setAnalyticsPeriod"]',
                    title: 'Time Period',
            ...
            'image-bank': [
                ...
                {
                    selector: '[onclick*="openImageUpload"]',
                    title: 'Upload Images',
                    description: 'Upload photos from your device or drag and drop.',
                },
                {
                    selector: '[onclick*="selectFolder"]',
                    title: 'Folders',
                    description: 'Organize images into folders for easy access.',
                },
            ],
            calendar: [
                {
                    ...
                    title: 'Calendar View',
                    description: 'Your events, deadlines, and milestones on a monthly calendar.',
                },
                {
                    selector: '.calendar-view-btn',
                    title: 'View Toggle',
                    description: 'Switch between Month, Week, and Day views.',
                },
                {
                    selector: '[onclick*="addCalendarEvent"]',
                    title: 'Add Events',
                    description: 'Click any day or use this button to add new events.',
                },
            ],
        };
    ...
    async deleteWebhookEndpoint(id) {
        if (
            !(await modals.confirm('Delete this webhook endpoint?', {
            ...
            <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
                <form id="add-shipping-profile-form" onsubmit="handlers.addShippingProfile(event)">
                    <div class="form-group">
                        ...
                        <inputinput id="hsa-profile-name" type="text" name="name" class="form-input" required placeholder="e.g., Standard Shipping">
                    </div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div class="form-group">
                            ...
                            <inputinput id="hsa-weight-oz" type="number" name="weightOz" class="form-input" step="0.1" min="0" placeholder="0">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="hsa-length">Length</label>
                            <inputinput id="hsa-length" type="number" name="length" class="form-input" step="0.1" min="0" placeholder="0">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="hsa-width">Width</label>
                            <inputinput id="hsa-width" type="number" name="width" class="form-input" step="0.1" min="0" placeholder="0">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="hsa-height">Height</label>
                            <inputinput id="hsa-height" type="number" name="height" class="form-input" step="0.1" min="0" placeholder="0">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="form-group">
                            ...
                            <inputinput id="hsa-domestic-cost" type="number" name="domesticCost" class="form-input" step="0.01" min="0" placeholder="0.00">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="hsa-international-cost">International Cost ($)</label>
                            <inputinput id="hsa-international-cost" type="number" name="internationalCost" class="form-input" step="0.01" min="0" placeholder="Leave blank for none">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="hsa-free-shipping-over">Free Shipping Over ($)</label>
                            <inputinput id="hsa-free-shipping-over" type="number" name="freeShippingThreshold" class="form-input" step="0.01" min="0" placeholder="Leave blank for none">
                        </div>
                    </div>
    ...
    updateServiceTypeOptions: function (carrier, mode) {
        ...
        select.innerHTML =
            sanitizeHTML('<option value="">Select Service</option>') + // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
            ...
            <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
                <form id="edit-shipping-profile-form" onsubmit="handlers.updateShippingProfile(event, '${profileId}')">
                    <div class="form-group">
                        ...
                        <inputinput id="hsa-profile-name-2" type="text" name="name" class="form-input" required value="${escapeHtml(profile.name)}">
                    </div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div class="form-group">
                            ...
                            <inputinput id="hsa-weight-oz-2" type="number" name="weightOz" class="form-input" step="0.1" min="0" value="${profile.weight_oz || 0}">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="form-group">
                            ...
                            <inputinput id="hsa-domestic-cost-2" type="number" name="domesticCost" class="form-input" step="0.01" min="0" value="${profile.domestic_cost || 0}">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="hsa-international-cost-2">International Cost ($)</label>
                            <inputinput id="hsa-international-cost-2" type="number" name="internationalCost" class="form-input" step="0.01" min="0" value="${profile.international_cost || ''}">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="hsa-free-shipping-over-2">Free Shipping Over ($)</label>
                            <inputinput id="hsa-free-shipping-over-2" type="number" name="freeShippingThreshold" class="form-input" step="0.01" min="0" value="${profile.free_shipping_threshold || ''}">
                        </div>
                    </div>
    ...
    deleteShippingProfile: async function (profileId) {
        if (
            !(await modals.confirm('Delete this shipping profile?', {
    ...
    bulkTogglePlatform: async function (platform, enable) {
        try {
            ...
            const presets = allPresets.filter(
                (a) => a.platform === platform || (a.platform === 'all' && platform !== 'all'),
            );
            let count = 0;
            await api.ensureCSRFToken();
            ...
            toast.success(
                `${count} ${platform} automation${count !== 1 ? 's' : ''} ${enable ? 'enabled' : 'disabled'}`,
            );
    ...
    filterAutomationPlatform: function (platform) {
        ...
        try {
            localStorage.setItem('vaultlister_automation_platform_filter', platform);
        } catch (e) {
    ...
    toggleAutomationSelect: function (ruleId, checked) {
        ...
        if (checked && !selected.includes(ruleId)) selected.push(ruleId);
            ...
            const idx = selected.indexOf(ruleId);
    ...
    bulkScheduleAutomations: function () {
        ...
        modals.show(`
            ...
            <div class="modal-body">
                ...
                <selectselect class="form-select" id="bulk-schedule-select">
    ...
    sortAutomations: function (value) {
        ...
        try {
            localStorage.setItem('vaultlister_automation_sort', value);
        } catch (_) {}
        if (store.state.currentPage === 'automations') {
            const pageContent = pages.automations();
            document.querySelector('.page-content').innerHTML = sanitizeHTML(pageContent); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
        ...
        modals.show(`
            ...
            <div class="modal-body">
                <div class="mb-4">
                    ...
                    <selectselect class="form-select" id="schedule-preset" onchange="(() => { const c = document.getElementById('schedule-custom-row'); const ci = document.getElementById('schedule-cron-custom'); if (this.value === 'custom') { c.style.display = 'block'; } else { c.style.display = 'none'; ci.value = this.value; } })()">
                ...
                <div id="schedule-custom-row" style="display: ${isCustom ? 'block' : 'none'};" class="mb-4">
                    ...
                    <inputinput type="text" class="form-input" id="schedule-cron-custom" value="${escapeHtml(currentSchedule || '')}" placeholder="0 */4 * * *">
    ...
    updateAutomationNotifPref: function (key, value) {
        ...
        try {
            localStorage.setItem('vaultlister_automation_notif_prefs', JSON.stringify(current));
        } catch (e) {}
        // Persist to backend
        api.ensureCSRFToken()
            .then(() => api.post('/automations/notification-prefs', current))
            .catch(() => {});
    ...
    showAutomationRunHistory: function (ruleName, ruleId) {
        ...
        const ruleRuns = runHistory.filter(
            (r) => r.automation_name === ruleName || r.automation_id === ruleId || r.action === ruleName,
        );
            ...
            ruleRuns.length === 0
                ...
                : ruleRuns
                      ...
                      .map((r) => {
                          ...
                          const items =
                              r.items_processed != null ? `${r.items_succeeded || 0}/${r.items_processed}` : '—';
                              ...
                              const lsc =
                                  l.status === 'success'
                                      ...
                                      : l.status === 'failure'
                                        ...
                                        : 'var(--warning-600)';
                              return (
                                  '<tr><td style="white-space:nowrap;font-size:12px;">' +
            ...
            modals.show(`
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <inputinput type="text" id="shopify-domain-input" class="form-input" placeholder="mystore.myshopify.com" autocomplete="off">
                    ...
                    <form id="connect-shop-form" onsubmit="handlers.submitShopConnection(event, 'shopify')">
                        <div class="form-group">
                            ...
                            <inputinput id="hsa-access-token-manual" type="password" name="apiKey" class="form-input" placeholder="shpat_..." autocomplete="off">
                        </div>
                ...
                <div>
                    ...
                    <form id="connect-shop-form" onsubmit="handlers.submitShopConnection(event, '${platform}')">
                        ...
                        <div class="form-group">
                            ...
                            <inputinput id="hsa-api-key-optional" type="password" name="apiKey" class="form-input" placeholder="Leave blank if not available" autocomplete="off">
                        </div>
    ...
    disconnectShop: async function (platform) {
        if (
            !(await modals.confirm(`Disconnect ${platform}?`, {
    ...
    connectOAuth: async function (platform, shopDomain = null) {
        try {
            ...
            const url = shopDomain
                ? `/oauth/authorize/${platform}?shop=${encodeURIComponent(shopDomain)}`
    ...
    showAccountModal: function () {
        ...
        modals.show(
            ...
            <div class="form-group">
                ...
                <inputinput id="hsa-account-created" type="text" class="form-input" value="${user.created_at ? new Date(user.created_at).toLocaleDateString() : 'N/A'}" disabled>
            </div>
            ...
            <div class="form-group">
                ...
                <inputinput type="password" class="form-input" id="account-confirm-password" placeholder="Confirm new password" autocomplete="new-password">
            </div>
        `,
            `
            <button class="btn btn-secondary" onclick="modals.close()">Cancel</button>
    ...
    saveSettings: async function () {
        ...
        if (
            timezone &&
            timezone.value !== (store.state.userTimezone || Intl.DateTimeFormat().resolvedOptions().timeZone)
        ) {
        ...
        if (
            !(await modals.confirm('Save these changes?\n\n' + summary, {
                title: 'Save Settings',
                confirmText: 'Save',
            }))
        ) {
    ...
    revokeAllSessions: async function () {
        if (
            !(await modals.confirm('Sign out all other sessions? Only your current session will remain active.', {
                danger: true,
            }))
        )
            return;
    ...
    deleteAccount: async function () {
        if (
            !(await modals.confirm(
                ...
                { title: 'Delete Account', confirmText: 'Continue', danger: true },
            ))
        ) {
            return;
        }
        if (
            !(await modals.confirm(
                'This will permanently delete all your data including listings, inventory, and sales history.',
                { title: 'Final Confirmation', confirmText: 'Delete My Account', danger: true },
            ))
        ) {
            return;
        }
            ...
            modals.show(`
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <selectselect id="deletion-reason" class="form-select">
    ...
    resetSettingsSection: async function (section) {
        ...
        if (
            !(await modals.confirm(
                `Reset all ${sectionName} settings to their default values? This action cannot be undone.`,
                { title: `Reset ${sectionName}`, confirmText: 'Reset' },
            ))
        ) {
            return;
        }
    ...
    updateSizeOptions: function (sizeType) {
        ...
        sizeSelect.innerHTML =
            sanitizeHTML('<option value="">Select size...</option>') + // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
    ...
    navigateFromNotification: function (notifId, typeOrLink) {
        // Mark as read first
        const notifications = store.state.notifications.map((n) => (n.id === notifId ? { ...n, read: true } : n));
        store.setState({ notifications });
        handlers.updateTabBadge();
    ...
    deleteNotification: async function (notificationId) {
        if (
            await modals.confirm('Are you sure you want to delete this notification?', {
        ...
        try {
            ...
            const handleMessage = async (event) => {
                ...
                if (event.data && event.data.type === 'email-oauth-success') {
                    ...
                    store.setState({ emailConnecting: false });
                    toast.success(
                        `${event.data.provider === 'outlook' ? 'Outlook' : 'Gmail'} connected: ${event.data.email}`,
                    );
                    ...
                    if (store.state.currentPage === 'receipt-parser' || store.state.currentPage === 'connections') {
                        const pageContent =
                            store.state.currentPage === 'connections' ? pages.connections() : pages.receiptParser();
    ...
    connectOutlook: async function () {
        try {
            ...
            if (!authUrl) {
                store.setState({ emailConnecting: false });
                toast.error(
                    'Outlook OAuth is not configured. Set OUTLOOK_CLIENT_ID and OUTLOOK_CLIENT_SECRET in Railway.',
                );
                return;
            }
            ...
            if (error.message?.includes('not configured') || error.message?.includes('OUTLOOK_CLIENT_ID')) {
                toast.error(
                    'Outlook OAuth is not configured yet. Set OUTLOOK_CLIENT_ID and OUTLOOK_CLIENT_SECRET in Railway.',
                );
            } else {
    ...
    disconnectEmailAccount: async function (accountId) {
        if (
            !(await modals.confirm('Disconnect this email account? Email syncing will stop.', {
    ...
    loadTeams: async function () {
        ...
        try {
            ...
            contentEl.innerHTML = sanitizeHTML(
                `<p style="color: var(--error);">Error loading teams: ${error.message}</p>`,
            ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
        }
    },
    ...
    removeTeamMember: async function (teamId, memberId) {
        if (
            !(await modals.confirm('Remove this member from the team?', {
                title: 'Remove Member',
                confirmText: 'Remove',
                danger: true,
            }))
    ...
    leaveTeam: async function (teamId) {
        if (
            !(await modals.confirm('Are you sure you want to leave this team?', {
                title: 'Leave Team',
                confirmText: 'Leave',
                danger: true,
            }))
    ...
    selectTeam: async function (teamId) {
        try {
            ...
            store.setState({
                activeTeam: {
                    ...response.team,
                    permissions: response.team?.permissions || {},
                    user_role: response.team?.user_role,
                },
    ...
    deleteTeam: async function (teamId) {
        if (
            !(await modals.confirm(
                ...
                { title: 'Delete Team', confirmText: 'Delete Team', danger: true },
            ))
        )
            return;
        try {
    ...
    deletePushSubscription: async function (subscriptionId) {
        if (
            !(await modals.confirm('Remove this device from push notifications?', {
                title: 'Remove Device',
                confirmText: 'Remove',
                danger: true,
            }))
    ...
    showAccountUsage: function () {
        ...
        const totalSales = orders.filter(
            (o) => o.status === 'completed' || o.status === 'shipped' || o.status === 'delivered',
        ).length;
        const totalRevenue = orders
            .filter((o) => o.status === 'completed' || o.status === 'shipped' || o.status === 'delivered')
            .reduce((s, o) => s + (o.sale_price || o.price || 0), 0);
                    ...
                    <div class="platform-usage-list">
                        ${platformEntries
                            .map(([platform, count]) => {
                                ...
                                return (
                                    '<div class="platform-usage-row">' +
                                    '<div class="platform-usage-name">' +
                                    escapeHtml(platform) +
                                    '</div>' +
                                    '<div class="priority-bar-track">' +
                                    '<div class="priority-bar priority-bar-primary" style="width: ' +
                                    pct +
                                    '%"></div>' +
                                    '</div>' +
                                    '<span class="platform-usage-count">' +
                                    count +
                                    ' items (' +
                                    pct +
                                    '%)</span>' +
    ...
    showDataAudit: async function () {
        try {
            ...
            toast.error('Failed to load data audit');
        }
    },

    clearCache: function () {
        const cacheKeys = Object.keys(localStorage).filter(
            (k) => k.startsWith('cache_') || k.startsWith('vl_cache_') || k.startsWith('search_cache_'),
        );
    ...
    clearDrafts: function () {
        const draftKeys = Object.keys(localStorage).filter(
            (k) => k.startsWith('draft_') || k.startsWith('vl_draft_') || k.startsWith('listing_draft_'),
        );
    ...
    showTosHistory: async function () {
        try {
            ...
            toast.error('Failed to load ToS history');
        }
    },
                ...
                <div class="modal-body">
                    <div style="display: grid; gap: 12px;">
                        <div class="form-group">
                            ...
                            <inputinput type="text" id="edit-landing-slug" class="form-input" value="${escapeHtml(page.slug || '')}">
                        ...
                        <div class="form-group">
                            ...
                            <inputinput type="text" id="edit-landing-title" class="form-input" value="${escapeHtml(page.title || '')}">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="edit-landing-desc">Description</label>
                            <textareatextarea id="edit-landing-desc" class="form-input" style="resize: vertical; min-height: 100px;">${escapeHtml(page.description || '')}</textarea>
                        </div>
                    </div>
    ...
    deleteLandingPage: async function (pageId) {
        if (
            !(await modals.confirm('Delete this landing page?', {
    ...
    showAffiliateTiers: async function () {
        try {
            ...
            toast.error('Failed to load affiliate tiers');
        }
    },
    ...
    clearCache() {
        Object.keys(localStorage)
            .filter((k) => k.startsWith('vl_cache_'))
    ...
    deleteAllUserData() {
        modals.show(
            `<div class="modal-header"><h3>Delete All Data</h3><button class="modal-close" aria-label="Close" onclick="modals.close()"><span aria-hidden="true">&times;</span></button></div><div class="modal-body"><p>This action is irreversible. Please contact support to delete your account.</p></div>`,
        );
    },
    ...
    showCreateCustomAutomation: function () {
        ...
        modals.show(
            ...
            <div class="modal-body" style="max-height:70vh;overflow-y:auto;">
                <div class="form-group mb-4"><label class="form-label" for="custom-auto-name">Automation Name</label><inputinput type="text" id="custom-auto-name" class="form-input" placeholder="e.g., Weekly Price Drop for Stale Items"></div>
                ...
                <div class="form-group mb-4"><label class="form-label" for="custom-auto-desc">Description</label><textareatextarea id="custom-auto-desc" class="form-input" rows="2" placeholder="What does this automation do?"></textarea></div>
    ...
    exportAutomationHistoryCSV: function () {
        ...
        runs.forEach((r) => {
            rows.push(
                [
                    escCSV(r.started_at || r.timestamp),
    ...
    switchInventoryTab: function (tabName) {
        ...
        if (tabName === 'analytics' && !store.state.inventoryAnalytics) {
            const _analyticsErr =
                '<div class="text-center py-8 text-gray-500">Unable to load analytics. Try refreshing.</div>';
    ...
    _renderInventoryAnalyticsContent: function () {
        ...
        return (
            '<div class="flex justify-between items-center mb-4"><h3 class="text-lg font-semibold">' +
            components.icon('bar-chart-2', 20) +
            ' Inventory Analytics</h3><div class="flex gap-2"><button class="btn btn-ghost btn-sm" onclick="handlers.showSupplierManager()">' +
            components.icon('truck', 14) +
            ' Suppliers</button><button class="btn btn-ghost btn-sm" onclick="handlers.showCategoryManager()">' +
            components.icon('folder', 14) +
            ' Manage Categories</button></div></div>' +
            '<div class="grid grid-cols-4 gap-4 mb-6"><div class="card"><div class="card-body text-center"><div class="text-2xl font-bold" style="color:var(--primary-600);">' +
            (overall.sell_through_rate || 0).toFixed(1) +
            '%</div><div class="text-xs text-gray-500">Sell-Through Rate</div></div></div><div class="card"><div class="card-body text-center"><div class="text-2xl font-bold" style="color:var(--success);">' +
            (overall.avg_days_to_sell || 0).toFixed(0) +
            '</div><div class="text-xs text-gray-500">Avg Days to Sell</div></div></div><div class="card"><div class="card-body text-center"><div class="text-2xl font-bold" style="color:var(--warning-600);">' +
            (overall.margin_pct || 0).toFixed(1) +
            '%</div><div class="text-xs text-gray-500">Avg Margin</div></div></div><div class="card"><div class="card-body text-center"><div class="text-2xl font-bold" style="color:' +
            ((overall.total_profit || 0) >= 0 ? 'var(--success)' : 'var(--error)') +
            ';">$' +
            (overall.total_profit || 0).toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
            }) +
            '</div><div class="text-xs text-gray-500">Total Profit</div></div></div></div>' +
            ...
            ' Profit & Loss Timeline</h3><button class="btn btn-ghost btn-sm" onclick="handlers.loadPLTimeline()">' +
            components.icon('refresh-cw', 14) +
            ' Load</button></div><div id="pl-timeline-chart">' +
            (store.state.plTimeline
                ? handlers._renderPLChart(store.state.plTimeline)
                : '<p class="text-gray-500 text-sm text-center py-4">Click Load to fetch monthly P&L data</p>') +
            '</div></div></div>' +
            '<div class="card mb-6"><div class="card-body"><h3 class="text-md font-semibold mb-3">' +
                  ...
                  agingBuckets
                      .map((b) => {
                          ...
                          return (
                              ...
                              ';height:' +
                              Math.max(pct, 4) +
                              '%;border-radius:4px 4px 0 0;margin:0 2px;position:relative;"><span style="position:absolute;top:-18px;left:50%;transform:translateX(-50%);font-size:11px;font-weight:600;">' +
                              b.count +
                              '</span></div><div style="font-size:10px;color:var(--gray-500);margin-top:4px;">' +
                              b.label +
                              '</div><div style="font-size:9px;color:var(--gray-400);">$' +
                      ...
                      .join('') +
                  '</div>'
                : '<p class="text-gray-500 text-sm text-center py-4">No aging data yet</p>') +
            '</div></div>' +
            '<div class="grid grid-cols-2 gap-4 mb-6"><div class="card"><div class="card-body"><h3 class="text-md font-semibold mb-3">' +
            components.icon('trending-up', 18) +
                ...
                ? '<table class="table table-sm"><thead><tr><th>Category</th><th>Total</th><th>Sold</th><th>Rate</th><th>Avg Days</th></tr></thead><tbody>' +
                  sellThrough
                      .map(
                          (s) =>
                              '<tr><td>' +
                              escapeHtml(s.category || 'Uncategorized') +
                              '</td><td>' +
                              s.total +
                              '</td><td>' +
                              s.sold +
                              ...
                              (s.sell_rate >= 50
                                  ...
                                  : s.sell_rate >= 25
                                    ...
                                    : 'var(--error)') +
                              ';font-weight:600;">' +
                              (s.sell_rate || 0).toFixed(1) +
                              '%</td><td>' +
                              (s.avg_days_to_sell ? s.avg_days_to_sell.toFixed(0) + 'd' : '—') +
                              '</td></tr>',
                      )
                      .join('') +
                  '</tbody></table>'
                : '<p class="text-gray-500 text-sm">No data yet</p>') +
            '</div></div><div class="card"><div class="card-body"><h3 class="text-md font-semibold mb-3">' +
                  ...
                  margins
                      .map(
                          (m) =>
                              ...
                              '%</td><td style="color:' +
                              ((m.total_profit || 0) >= 0 ? 'var(--success)' : 'var(--error)') +
                              ';">$' +
                              (m.total_profit || 0).toFixed(0) +
                              '</td></tr>',
                      )
                      .join('') +
                  '</tbody></table>'
            ...
            (sellThrough.length > 1
                          ...
                          sorted
                              ...
                              .map((c, i) => {
                                  ...
                                  return (
                                      ...
                                      '<span style="width:90px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
                                      escapeHtml(c.category || 'Uncategorized') +
                                      '</span>' +
                                      ...
                                      ';border-radius:var(--radius-sm);"></div></div>' +
                                      '<span style="width:55px;text-align:right;font-weight:600;">' +
                                      (c.total || 0) +
                                      ' (' +
                                      pct +
                                      '%)</span></div>'
                          ...
                          [...margins]
                              ...
                              .map((m, i) => {
                                  ...
                                  return (
                                      ...
                                      ';border-radius:var(--radius-sm);"></div></div>' +
                                      '<span style="width:60px;text-align:right;font-weight:600;color:' +
                                      (isPositive ? 'var(--success)' : 'var(--error)') +
                                      ';">$' +
                                      Math.round(m.total_profit || 0).toLocaleString() +
                                      '</span></div>'
                                  );
                              })
                              .join('') +
                ...
                : '') +
            (deadStock.length > 0
                ? '<div class="card mb-6"><div class="card-body"><h3 class="text-md font-semibold mb-3" style="color:var(--error);">' +
                  ...
                  deadStock
                      ...
                      .map(
                          (d) =>
                              ...
                              '</td><td>' +
                              escapeHtml(d.sku || '—') +
                              '</td><td style="color:var(--error);">' +
                              d.days_old +
                              'd</td><td>$' +
                              (d.list_price || 0).toFixed(0) +
                              '</td></tr>',
                      )
                      .join('') +
                  '</tbody></table></div></div>'
            ...
            ' Price Suggestions</h3><button class="btn btn-ghost btn-sm" onclick="handlers.loadPriceSuggestions()">' +
            components.icon('refresh-cw', 14) +
            ' Load</button></div><div id="price-suggestions-content">' +
            (store.state.priceSuggestions
                ? handlers._renderPriceSuggestions(store.state.priceSuggestions)
                : '<p class="text-gray-500 text-sm text-center py-4">Click Load to get AI-powered pricing recommendations for aging inventory</p>') +
            '</div></div></div>'
        );
    },
    ...
    _renderPLChart: function (salesData) {
        ...
        const gridLines = [0, 0.25, 0.5, 0.75, 1]
            .map((pct) => {
                ...
                return (
                    ...
                    '" y2="' +
                    y +
                    '" stroke="var(--gray-200)" stroke-dasharray="4,4"/>' +
                    '<text x="' +
                    (padL - 6) +
                    '" y="' +
                    (y + 4) +
                    '" text-anchor="end" font-size="10" fill="var(--gray-400)">$' +
                    Math.round(val) +
                    '</text>'
                );
            })
            .join('');
        const xLabels = salesData
            .map(
                (d, i) =>
                    '<text x="' +
                    toX(i) +
                    '" y="' +
                    (height - 8) +
                    '" text-anchor="middle" font-size="10" fill="var(--gray-500)">' +
                    (d.period || '').slice(0, 7) +
                    '</text>',
            )
            .join('');
        ...
        return (
            '<div class="flex gap-4 mb-2"><span class="text-xs"><span style="display:inline-block;width:12px;height:3px;background:var(--primary-500);vertical-align:middle;margin-right:4px;border-radius:2px;"></span>Revenue ($' +
            totalRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 }) +
            ')</span><span class="text-xs"><span style="display:inline-block;width:12px;height:3px;background:var(--success);vertical-align:middle;margin-right:4px;border-radius:2px;"></span>Profit ($' +
            ...
            (minVal < 0
                ? '<line x1="' +
                  ...
                  '" y2="' +
                  zeroY +
                  '" stroke="var(--gray-400)" stroke-width="1"/>'
                : '') +
            '<polyline points="' +
            revPoints +
            '" fill="none" stroke="var(--primary-500)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
            '<polyline points="' +
            profPoints +
            '" fill="none" stroke="var(--success)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
            salesData
                .map(
                    (d, i) =>
                        '<circle cx="' +
                        ...
                        '" cy="' +
                        toY(revenues[i]) +
                        '" r="3" fill="var(--primary-500)"/><circle cx="' +
                        toX(i) +
                        '" cy="' +
                        toY(profits[i]) +
    ...
    showCreateExperiment: function () {
        ...
        const ruleOptions = enabledRules
    ...
    _renderExperimentsList: function (experiments) {
        ...
        return experiments
            .map((exp) => {
                const statusColor =
                    exp.status === 'running'
                        ...
                        : exp.status === 'paused'
                          ...
                          : 'var(--gray-500)';
                const statusIcon =
                    exp.status === 'running' ? 'play' : exp.status === 'paused' ? 'pause' : 'check-circle';
                ...
                return (
                    '<div class="card mb-3" style="border-left:3px solid ' +
                    statusColor +
                    ';"><div class="card-body">' +
                    '<div class="flex justify-between items-start mb-2"><div><h3 class="font-semibold">' +
                    components.icon(statusIcon, 14) +
                    ' ' +
                    escapeHtml(exp.name || 'Experiment') +
                    '</h3>' +
                    ...
                    '% success</div>' +
                    (bs.avg_items != null
                        ? '<div class="text-xs text-gray-400">' + Math.round(bs.avg_items) + ' avg items</div>'
                        : '') +
                    '<div class="text-xs text-gray-400">' +
                    escapeHtml(exp.base_name || exp.base_rule_id || '') +
                    '</div></div>' +
                    ...
                    '% success</div>' +
                    (vs.avg_items != null
                        ? '<div class="text-xs text-gray-400">' + Math.round(vs.avg_items) + ' avg items</div>'
                        : '') +
                    '<div class="text-xs text-gray-400">' +
                    escapeHtml(exp.variant_name || exp.variant_rule_id || '') +
                    '</div></div></div>' +
                    (exp.winner
                        ? '<div class="text-sm mb-2" style="color:var(--success);font-weight:600;">' +
                          components.icon('award', 14) +
                          ' Winner: ' +
                          exp.winner.toUpperCase() +
                          '</div>'
                        : '') +
                    (exp.status !== 'completed'
                        ? '<div class="flex gap-2">' +
                          '<button class="btn btn-xs btn-success" onclick="handlers.completeExperiment(\'' +
                          exp.id +
                          "', 'base')\">" +
                          components.icon('check', 12) +
                          ' A Wins</button>' +
                          '<button class="btn btn-xs btn-warning" onclick="handlers.completeExperiment(\'' +
                          exp.id +
                          "', 'variant')\">" +
                          components.icon('check', 12) +
                          ' B Wins</button>' +
                          '<button class="btn btn-xs btn-ghost" onclick="handlers.completeExperiment(\'' +
                          exp.id +
                          "', 'inconclusive')\">Inconclusive</button>" +
                          '<button class="btn btn-xs btn-ghost" onclick="handlers.pauseExperiment(\'' +
                          exp.id +
                          "', '" +
                          (exp.status === 'paused' ? 'running' : 'paused') +
                          '\')">' +
                          components.icon(exp.status === 'paused' ? 'play' : 'pause', 12) +
                          ' ' +
    ...
    showImportAutomationRules: function () {
        modals.show(
            '<div class="modal-header"><h2 class="modal-title">' +
                components.icon('upload', 20) +
                ' Import Automation Rules</h2><button class="modal-close" aria-label="Close" onclick="modals.close()">' +
                components.icon('close') +
    ...
    _renderForecastContent: function (data) {
        ...
        return (
            '<div class="grid grid-cols-4 gap-3 mb-4"><div class="card"><div class="card-body text-center py-2"><div class="text-xl font-bold">' +
            (overall.totalActive || 0) +
            '</div><div class="text-xs text-gray-500">Active Items</div></div></div><div class="card"><div class="card-body text-center py-2"><div class="text-xl font-bold">$' +
            Math.round(overall.totalValue || 0).toLocaleString() +
            '</div><div class="text-xs text-gray-500">Inventory Value</div></div></div><div class="card"><div class="card-body text-center py-2"><div class="text-xl font-bold" style="color:var(--error);">' +
            (overall.restockAlerts || 0) +
            ...
            forecasts
                .map((f) => {
                    ...
                    return (
                        ...
                        f.active_count +
                        '</td><td>$' +
                        Math.round(f.active_value).toLocaleString() +
                        '</td><td>' +
                        f.monthly_velocity.toFixed(1) +
                        '/mo</td><td style="color:' +
                        hc +
                        ';font-weight:600;">' +
                        (f.days_of_supply != null ? f.days_of_supply + 'd' : '—') +
                        '</td><td><span class="badge" style="background:' +
                        hc +
                        '20;color:' +
                        hc +
                        ';">' +
                        (healthLabels[f.health] || f.health) +
                        '</span></td><td>' +
                        f.projected_quarterly_sales +
                        '</td></tr>'
                    );
                })
                .join('') +
    ...
    showTemplateMarketplace: function () {
        modals.show(
            '<div class="modal-header"><h2 class="modal-title">' +
                ...
                ' Automation Templates</h2><button class="modal-close" aria-label="Close" onclick="modals.close()">' +
                components.icon('close') +
                '</button></div><div class="modal-body" style="max-height:65vh;overflow-y:auto;"><div id="template-marketplace"><p class="text-gray-500 text-sm text-center py-4">Loading templates...</p></div></div>',
        );
                        ...
                        versions.length === 0
                            ...
                            : '<table class="table table-sm"><thead><tr><th>Ver</th><th>Name</th><th>Platform</th><th>Changes</th><th>Date</th><th></th></tr></thead><tbody>' +
                              versions
                                  .map(
                                      (v) =>
                                          '<tr><td><span class="badge badge-sm">v' +
                                          v.version +
                                          '</span></td>' +
                                          '<td>' +
                                          escapeHtml(v.name) +
                                          ...
                                          '</td>' +
                                          '<td><button class="btn btn-xs btn-ghost" onclick="handlers.rollbackRule(\'' +
                                          ruleId +
                                          "', '" +
                                          v.id +
                                          "', " +
    ...
    rollbackRule: async function (ruleId, versionId, versionNum) {
        if (
            !confirm(
                'Rollback this rule to version ' + versionNum + '? Current settings will be saved as a new version.',
            )
        )
            return;
        try {
    ...
    showCategoryManager: async function () {
        ...
        const renderList = (cats) => {
            if (!cats || cats.length === 0)
                return '<p class="text-gray-500 text-sm text-center py-4">No categories yet. Add one below.</p>';
            return (
                '<div class="flex flex-col gap-2">' +
                cats
                    .map(
                        (c) =>
                            '<div class="flex items-center gap-3 p-2" style="border:1px solid var(--border);border-radius:var(--radius-sm);border-left:4px solid ' +
                            (c.color || 'var(--primary-500)') +
                            ';">' +
                            '<div class="flex-1"><span class="font-semibold text-sm">' +
                            escapeHtml(c.name) +
                            '</span>' +
                            '<span class="text-xs text-gray-400 ml-2">' +
                            (c.item_count || 0) +
                            ' items</span></div>' +
                            '<input aria-label="Color picker" type="color" value="' +
                            (c.color || 'var(--primary-500)') +
                            '" onchange="handlers.updateCategory(\'' +
                            c.id +
                            '\', { color: this.value })" style="width:28px;height:28px;border:none;cursor:pointer;" title="Change color">' +
                            '<button class="btn btn-xs btn-ghost" onclick="handlers.renameCategory(\'' +
                            c.id +
                            "', '" +
                            escapeHtml(c.name).replace(/\\/g, '\\\\').replace(/'/g, "\\'") +
                            '\')" title="Rename">' +
                            components.icon('edit-2', 12) +
                            '</button>' +
                            '<button class="btn btn-xs btn-ghost" style="color:var(--error);" onclick="handlers.deleteCategory(\'' +
                            c.id +
                            "', '" +
                            escapeHtml(c.name).replace(/\\/g, '\\\\').replace(/'/g, "\\'") +
                            '\')" title="Delete">' +
    ...
    showScheduleCalendar: function () {
        ...
        const cellContent = (dow, slotIdx) => {
            ...
            return (
                '<td style="padding:4px;border:1px solid var(--gray-100);background:var(--primary-50,var(--blue-50));">' +
                entries
                    .slice(0, 3)
                    .map(
                        (e) =>
                            '<div style="font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100px;" title="' +
                            escapeHtml(e.name) +
                            ' (' +
                            e.hour +
                            ':00)">' +
                            '<span style="color:var(--primary-500);">●</span> ' +
                            escapeHtml(e.name.slice(0, 12)) +
                            '</div>',
                    )
                    .join('') +
                (entries.length > 3
                    ? '<div style="font-size:9px;color:var(--gray-400);">+' + (entries.length - 3) + ' more</div>'
                    : '') +
                '</td>'
            );
        };
                ...
                <table class="table table-sm" style="table-layout:fixed;">
                    ...
                    <tbody>
                        ${hours
                            .map(
                                (h, si) =>
                                    '<tr><td style="font-size:11px;font-weight:600;color:var(--gray-500);padding:4px;">' +
                                    h +
                                    ':00</td>' +
                                    days.map((_, di) => cellContent(di, si)).join('') +
                                    '</tr>',
                            )
                            .join('')}
                    </tbody>
        ...
        modals.show(`
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <selectselect id="bulk-cat-select" class="form-select">
    ...
    showAutomationPerformance: function () {
        ...
        if (rules.length === 0) {
            toast.warning('No automation rules configured');
            return;
        }

        const perfData = rules
            .map((rule) => {
                const ruleRuns = runHistory.filter(
                    (r) => r.automation_name === rule.name || r.automation_id === rule.id || r.action === rule.name,
                );
                ...
                const avgItems =
                    total > 0 ? Math.round(ruleRuns.reduce((s, r) => s + (r.items_processed || 0), 0) / total) : 0;
                const avgDuration =
                    total > 0 ? Math.round(ruleRuns.reduce((s, r) => s + (r.duration_ms || 0), 0) / total / 1000) : 0;
                return {
                    name: rule.name,
                    ...
                    <div class="flex flex-col gap-1">
                        ${perfData
                            ...
                            .map(
                                (d) =>
                                    '<div class="flex items-center gap-2" style="font-size:12px;">' +
                                    '<span style="width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' +
                                    escapeHtml(d.name) +
                                    '">' +
                                    escapeHtml(d.name) +
                                    '</span>' +
                                    ...
                                    (d.successRate >= 80
                                        ...
                                        : d.successRate >= 50
                                          ? 'var(--warning-500)'
                                          : 'var(--error)') +
                                    ';border-radius:var(--radius-sm);display:flex;align-items:center;padding-left:4px;">' +
                                    '<span style="color:white;font-size:10px;font-weight:600;">' +
                                    d.total +
                                    '</span></div></div>' +
                                    '<span style="width:40px;text-align:right;font-weight:600;color:' +
                                    (d.successRate >= 80
                                        ? 'var(--success)'
                                        : d.successRate >= 50
                                          ? 'var(--warning-600)'
                        ...
                        ${perfData
                            .map(
                                (d) =>
                                    ...
                                    '</td>' +
                                    '<td><span class="badge badge-sm" style="background:' +
                                    (d.enabled
                                        ? 'var(--success)20;color:var(--success)'
                                        : 'var(--gray-400)20;color:var(--gray-500)') +
                                    ';">' +
                              ...
                              suppliers
                                  .map(
                                      (s) =>
                                          ...
                                          (s.item_count || 0) +
                                          '</td><td>$' +
                                          (s.avg_price || 0).toFixed(2) +
                                          '</td><td class="flex gap-1"><button class="btn btn-xs btn-ghost" onclick="handlers.showSupplierPerformance(\'' +
                                          s.id +
                                          "', '" +
                                          escapeHtml(s.name).replace(/\\/g, '\\\\').replace(/'/g, "\\'") +
                                          '\')" title="Performance">' +
                                          components.icon('trending-up', 12) +
                                          '</button><button class="btn btn-xs btn-danger" onclick="handlers.deleteSupplier(\'' +
                                          s.id +
                                          "', '" +
                                          escapeHtml(s.name).replace(/\\/g, '\\\\').replace(/'/g, "\\'") +
                                          '\')">' +
    ...
    _renderDurationTrendsChart: function (trends) {
        ...
        function toY(v) {
            return padT + chartH - (v / maxDur) * chartH;
        }
        ...
        const xLabels = allDays
            .map(
                (d, i) =>
                    ...
                    '" y="' +
                    (height - 8) +
                    '" text-anchor="middle" font-size="9" fill="var(--gray-500)">' +
                    d.slice(5) +
                    '</text>',
            )
            .join('');
        ...
        const legend =
            ...
            names
                .map(
                    (name, ci) =>
                        '<span class="text-xs"><span style="display:inline-block;width:12px;height:3px;background:' +
                        colors[ci % colors.length] +
    ...
    _renderPriceSuggestions: function (suggestions) {
        ...
        return (
            '<div class="flex justify-end mb-2"><button class="btn btn-primary btn-sm" onclick="handlers.applyAllPriceSuggestions()">' +
            ...
            suggestions
                ...
                .map((s) => {
                    ...
                    return (
                        '<tr><td class="text-sm" style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
                        escapeHtml(s.title || '') +
                        '</td>' +
                        '<td>' +
                        (s.days_old || 0) +
                        'd</td>' +
                        '<td>$' +
                        (s.list_price || s.current_price || 0).toFixed(2) +
                        '</td>' +
                        ...
                        changePrefix +
                        'C$' +
                        (s.price_change || 0).toFixed(2) +
                        '</td>' +
                        '<td class="text-xs text-gray-500">' +
                        escapeHtml(s.reason || '') +
                        '</td>' +
                        '<td><button class="btn btn-xs btn-ghost" onclick="handlers.applySinglePriceSuggestion(\'' +
                        s.id +
                        "', " +
                        (s.suggested_price || 0) +
                        ', ' +
                        (s.list_price || s.current_price || 0) +
                        ')">' +
                        components.icon('check', 12) +
                        '</button></td></tr>'
                    );
            ...
            (suggestions.length > 20
                ? '<p class="text-xs text-gray-400 mt-2">Showing 20 of ' + suggestions.length + ' suggestions</p>'
                : '')
        );
    },
        ...
        modals.show(`
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <inputinput type="text" id="import-url-input" class="form-input" placeholder="https://example.com/rules.json">
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-ghost" onclick="modals.close()">Cancel</button>
    ...
    applyAllPriceSuggestions: async function () {
        ...
        if (suggestions.length === 0) {
            toast.warning('No suggestions to apply');
            return;
        }
        ...
        try {
            ...
            if (trends.length > 1) {
                ...
                const points = trends
                    ...
                    .join(' ');
                trendChart =
                    '<div class="mb-3"><div class="text-xs text-gray-500 mb-1">Avg Cost Trend (6 months)</div>' +
                    '<svg viewBox="0 0 ' +
                    w +
                    ' ' +
                    h +
                    '" style="width:100%;height:80px;"><polyline points="' +
                    points +
                    '" fill="none" stroke="var(--primary-500)" stroke-width="2"/></svg>' +
                    '<div class="flex justify-between text-xs text-gray-400"><span>' +
                    (trends[0]?.month || '') +
                    '</span><span>' +
                              ...
                              recent
                                  .map(
                                      (r) =>
                                          ...
                                          escapeHtml(r.title || '') +
                                          '</td><td>$' +
                                          (r.cost_price || 0).toFixed(0) +
                                          '</td><td>$' +
                                          (r.list_price || 0).toFixed(0) +
        ...
        return (
            ...
            templates
                .map((t) => {
                    ...
                    return (
                        '<div class="card"><div class="card-body"><div class="flex justify-between items-start mb-2"><h3 class="font-semibold text-sm">' +
                        escapeHtml(t.name) +
                        '</h3><span class="badge badge-sm">' +
                        escapeHtml(t.platform || 'all') +
                        '</span></div><p class="text-xs text-gray-500 mb-2">' +
                        escapeHtml(t.description || '') +
                        '</p><div class="flex items-center gap-2 mb-2"><span class="text-xs text-gray-400">' +
                        components.icon('user', 10) +
                        ' ' +
                        escapeHtml(t.author_name || 'Unknown') +
                        '</span><span class="text-xs text-gray-400">' +
                        components.icon('download', 10) +
                        ' ' +
                        (t.install_count || 0) +
                        '</span></div>' +
                        (tags.length > 0
                            ? '<div class="flex gap-1 mb-2">' +
                              ...
                              '</div>'
                            : '') +
                        '<button class="btn btn-xs btn-primary" onclick="handlers.installTemplate(\'' +
                        t.id +
                        "', '" +
                        escapeHtml(t.name).replace(/\\/g, '\\\\').replace(/'/g, "\\'") +
                        '\')">' +
                        components.icon('download', 12) +
    ...
    async openARPreview(itemId) {
            ...
            <div class="modal-body" style="padding:0;">
                ...
                <div style="padding:0.75rem 1rem;background:var(--gray-50);border-top:1px solid var(--gray-200);display:flex;align-items:center;gap:0.75rem;flex-wrap:wrap;">
                    ...
                    <div style="margin-left:auto;display:flex;gap:0.5rem;">
                        ...
                        ${
                            arSupported
                                ? `<button class="btn btn-sm btn-secondary" aria-label="Take snapshot" onclick="handlers._arTakeSnapshot()" style="min-height:44px;">
        ...
        requestAnimationFrame(async () => {
            try {
                if (arSupported) {
                    ...
                    if (container) {
                        container.style.background =
                            'repeating-conic-gradient(#ccc 0% 25%, #e5e5e5 0% 50%) 0 0 / 32px 32px';
                ...
                const loadingEl = document.getElementById('ar-preview-loading');
                if (loadingEl) {
                    loadingEl.innerHTML = sanitizeHTML(
                        `<div style="text-align:center;padding:1rem;color:var(--error-300);">${components.icon('alert-circle', 24)}<p class="text-sm mt-2">Could not start preview</p><p class="text-xs mt-1" style="color:var(--gray-400);">${escapeHtml(err.message || 'Unknown error')}</p></div>`,
                    ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                }
        ...
        modals.show(`
            ...
            <div style="padding: 16px;">
                ...
                <div class="form-group">
                    ...
                    <inputinput type="email" class="form-input" id="new-email-input" placeholder="you@example.com" required>
                </div>
