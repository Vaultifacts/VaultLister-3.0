...
function isValidImageUrl(url) {
    try {
        ...
        return ['https:', 'data:', 'blob:'].includes(parsed.protocol);
    } catch { return false; }
}

Object.assign(handlers, {  // nosemgrep: javascript.lang.security.insecure-object-assign.insecure-object-assign
            ...
            <div class="modal-body">
                <div class="price-drop-listing-info">
                    <div class="listing-preview">
                        <img src="${isValidImageUrl(listing.images?.[0]) ? escapeHtml(listing.images[0]) : 'https://placehold.co/60x60?text=Item'}" alt="${escapeHtml(listing.title)}">
                        <div>
            ...
            <div class="duplicate-card" data-id="${d.id}">
                <div class="duplicate-card-items">
                    <div class="duplicate-item-preview">
                        ${d.primary_images?.length && isValidImageUrl(d.primary_images[0]) ? `<img src="${escapeHtml(d.primary_images[0])}" alt="${escapeHtml(d.primary_title || 'Duplicate item')}" width="60" height="60" style="object-fit: cover;">` : '<div style="width:60px;height:60px;background:var(--gray-200);border-radius:8px;"></div>'}
                        <span>${escapeHtml(d.primary_title || 'Item 1')}</span>
                    </div>
                    <div class="duplicate-arrow">${components.icon('arrow-right', 24)}</div>
                    <div class="duplicate-item-preview">
                        ${d.duplicate_images?.length && isValidImageUrl(d.duplicate_images[0]) ? `<img src="${escapeHtml(d.duplicate_images[0])}" alt="${escapeHtml(d.duplicate_title || 'Duplicate item')}" width="60" height="60" style="object-fit: cover;">` : '<div style="width:60px;height:60px;background:var(--gray-200);border-radius:8px;"></div>'}
    ...
    saveMappingTemplate: async function() {
        ...
        if (store.state._lastImportMapping) {
            Object.assign(fieldMapping, store.state._lastImportMapping);  // nosemgrep: javascript.lang.security.insecure-object-assign.insecure-object-assign
        }
