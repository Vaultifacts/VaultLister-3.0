...
Object.assign(handlers, {
    ...
    removeImageBackground: async function (imageId, previewEl) {
        if (!imageId) {
            toast.error('No image ID — upload to Image Bank first');
            return;
        }
        try {
    ...
    renderVariations: function () {
        ...
        if (variations.length === 0) {
            variationsList.innerHTML = sanitizeHTML(
                '<p style="font-size: 13px; color: var(--gray-500); font-style: italic;">No variations added yet</p>',
            ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
            return;
        }
    ...
    updateVariation: function (variationId, value) {
        const variations = (store.state.itemVariations || []).map((v) => (v.id === variationId ? { ...v, value } : v));
        store.setState({ itemVariations: variations });
    },
    ...
    async addItem(event) {
        ...
        try {
            ...
            if (typeof gtag === 'function')
                gtag('event', 'add_item', { item_category: result.item?.category || 'unknown' });
    ...
    async updateItem(event, itemId) {
        ...
        if (removedIndices) {
            const indicesToRemove = removedIndices
                .split(',')
                .map((i) => parseInt(i, 10))
                .filter((i) => !isNaN(i));
        ...
        try {
            ...
            try {
                ...
                if (
                    putError.message &&
                    (putError.message.includes('404') ||
                        putError.message.includes('not found') ||
                        putError.message.includes('Not found'))
                ) {
    ...
    testAutomation: async function (automationId) {
        const allPresets = (window.AUTOMATION_PRESETS || []).map((p) => ({
            id: p.id,
            name: p.name,
            category: p.category,
        }));
    ...
    showAutomationHistory: async function () {
        ...
        try {
            ...
            const avgDuration =
                totalRuns > 0 ? Math.round(runs.reduce((sum, r) => sum + (r.duration_ms || 0), 0) / totalRuns) : 0;
                                        ...
                                        <div class="automation-run-status ${run.status}" style="margin-top: 2px;">
                                            ${
                                                run.status === 'success'
                                                    ...
                                                    : run.status === 'failed'
                                                      ? components.icon('x-circle', 20)
                                                      : run.status === 'partial'
                                                        ? components.icon('alert-circle', 20)
    ...
    showAutomationHistoryMock: function () {
        const mockRuns = [
            {
                id: '1',
                automation_name: 'Daily Closet Share',
                automation_type: 'sharing',
                status: 'success',
                ...
                items_succeeded: 52,
                items_failed: 0,
                result_message: 'Successfully shared 52 items to your closet',
            },
            {
                id: '2',
                automation_name: 'Send Offers to Likers',
                automation_type: 'offers',
                status: 'success',
                ...
                items_succeeded: 15,
                items_failed: 0,
                result_message: 'Sent 15 offers to likers',
            },
            {
                id: '3',
                automation_name: 'Follow Back',
                automation_type: 'engagement',
                status: 'partial',
                ...
                items_succeeded: 8,
                items_failed: 2,
                result_message: '8 users followed, 2 failed due to rate limit',
            },
            {
                id: '4',
                automation_name: 'Weekly Price Drop',
                automation_type: 'pricing',
                status: 'failed',
                ...
                items_succeeded: 0,
                items_failed: 0,
                error_message: 'API rate limit exceeded. Retry in 1 hour.',
                retry_count: 2,
            },
            {
                id: '5',
                automation_name: 'Relist Stale Items',
                ...
                items_succeeded: 8,
                items_failed: 0,
                result_message: 'Relisted 8 stale items with optimized titles',
            },
            {
                id: '6',
                automation_name: 'Community Share',
                automation_type: 'sharing',
                status: 'success',
                ...
                items_succeeded: 25,
                items_failed: 0,
                result_message: 'Shared 25 items from community closets',
            },
        ];
                            ...
                            <div class="automation-history-item p-4 hover:bg-gray-50 cursor-pointer"
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="automation-run-status ${run.status}" style="margin-top: 2px;">
                                        ${
                                            run.status === 'success'
                                                ? `<span style="color: var(--success-600, var(--green-600))">${components.icon('check-circle', 20)}</span>`
                                                : run.status === 'failed'
                                                  ? `<span style="color: var(--error-600, var(--error-600))">${components.icon('x-circle', 20)}</span>`
                                                  : run.status === 'partial'
                                                    ? `<span style="color: var(--warning-600, var(--primary-600))">${components.icon('alert-circle', 20)}</span>`
            ...
            <div class="modal-body">
                <div class="flex items-center gap-3 mb-6 pb-4 border-b" style="border-color: var(--gray-200);">
                    <div class="automation-run-status ${run.status}" style="width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; border-radius: var(--radius-lg); background: ${run.status === 'success' ? 'var(--success-50, var(--green-100))' : run.status === 'failed' ? 'var(--error-50, var(--error-50))' : 'var(--warning-50, var(--yellow-50))'};">
                        ${
                            run.status === 'success'
                                ? `<span style="color: var(--success-600, var(--green-600))">${components.icon('check-circle', 24)}</span>`
                                : run.status === 'failed'
                                  ? `<span style="color: var(--error-600, var(--error-600))">${components.icon('x-circle', 24)}</span>`
    ...
    clearAutomationHistory: async function () {
        if (
            !(await modals.confirm('Are you sure you want to clear all automation history? This cannot be undone.', {
                title: 'Clear History',
    ...
    bulkTag: async function (ids) {
        const tag = await modals.prompt('Enter tag to add to all selected items:', {
            title: 'Add Tag',
    ...
    exportSelectedItems: function (ids) {
        ...
        const visibleCols = (window.tablePrefs?.getVisibleColumns('inventory', defaultCols) || defaultCols).filter(
            (c) => INVENTORY_COL_MAP[c],
        );
    ...
    deleteItem: async function (itemId) {
        ...
        if (
            !(await modals.confirm(`Delete "${itemTitle}"?`, {
                title: 'Delete Item',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    deleteSelected: async function () {
        ...
        if (
            !(await modals.confirm(`Delete ${ids.length} item(s)?`, {
                title: 'Delete Items',
    ...
    archiveSelected: async function () {
        ...
        if (
            !(await modals.confirm(`Archive ${ids.length} item(s)?`, {
                title: 'Archive Items',
    ...
    duplicateSelected: async function () {
        ...
        if (
            !(await modals.confirm(`Duplicate ${ids.length} item(s)? New copies will be created as drafts.`, {
    ...
    bulkUpdatePrice: async function () {
        ...
        const input = await modals.prompt(
            `Adjust price for ${ids.length} item(s). Enter a percentage (e.g. +10%, -15%) or dollar amount (e.g. +5.00, -2.50):`,
            { title: 'Bulk Price Adjustment', placeholder: '+10% or -$2.50' },
        );
        if (!input) return;
    ...
    applyFilters: async function () {
        ...
        try {
            ...
            if (filterCount > 0) {
                toast.success(
                    `${filterCount} filter${filterCount > 1 ? 's' : ''} applied (${data.items.length} results)`,
                );
            }
        } catch (error) {
            toast.error(error.message);
    ...
    renderActiveFilters: function () {
        ...
        if (Object.keys(activeFilters).length === 0) {
            container.innerHTML = sanitizeHTML(
                '<div style="font-size: 12px; color: var(--gray-500); padding: 8px;">No active filters</div>',
            ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
            return;
        }
    ...
    showListingCharCount: function (inputEl, maxChars) {
        ...
        counter.style.color =
            len > maxChars ? 'var(--error)' : len > maxChars * 0.8 ? 'var(--warning-600)' : 'var(--gray-500)';
    ...
    archiveListing: async function (listingId) {
        ...
        if (
            !(await modals.confirm(`Archive listing "${listing.title}" on ${listing.platform}?`, {
    ...
    deleteListing: async function (listingId) {
        ...
        if (
            !(await modals.confirm(`Delete listing "${listing.title}" on ${listing.platform}? This cannot be undone.`, {
                ...
                <div class="price-drop-options">
                    ...
                    <div class="price-drop-presets">
                        ...
                        <button class="preset-btn" onclick="handlers.setPriceDropPreset('${listingId}', 10)">
                            ...
                            <span class="preset-price">C$${(currentPrice * 0.9).toFixed(2)}</span>
                        </button>
                        ...
                        <button class="preset-btn" onclick="handlers.setPriceDropPreset('${listingId}', 20)">
                            ...
                            <span class="preset-price">C$${(currentPrice * 0.8).toFixed(2)}</span>
                        </button>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="new-price-preview" id="new-price-preview">
                            ...
                            <span class="new-price">C$${(currentPrice * 0.9).toFixed(2)}</span>
                        </div>
                    </div>
    ...
    savePriceDropSchedule: async function (listingId, currentPrice) {
        ...
        try {
            ...
            if (timing === 'now') {
                const listings = store.state.listings.map((l) => (l.id === listingId ? { ...l, price: newPrice } : l));
    ...
    loadCompetitorPricing: async function (listingId) {
        ...
        try {
            ...
            if (container) {
                const recColor =
                    data.recommendation === 'competitive'
                        ? 'success'
                        : data.recommendation === 'underpriced'
                          ? 'warning'
                          : 'error';
                // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                container.innerHTML = sanitizeHTML(`
            ...
            if (container)
                container.innerHTML = sanitizeHTML('<span class="text-gray-400">No pricing data available</span>'); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
        }
    },
        ...
        try {
            ...
            if (container) {
                if (data.estimated_days !== null) {
                    const confColor =
                        data.confidence === 'high' ? 'success' : data.confidence === 'medium' ? 'warning' : 'gray';
                    // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                    container.innerHTML = sanitizeHTML(`
                        <div class="space-y-1">
                    ...
                    container.innerHTML = sanitizeHTML(
                        '<span class="text-gray-400">Not enough data to estimate</span>',
                    ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                }
            }
        } catch (error) {
    ...
    showProfitCalculator: function (itemId = null) {
        ...
        if (itemId) {
            item =
                store.state.inventory?.find((i) => i.id === itemId) ||
        ...
        if (profitBar && profitIndicator) {
            ...
            if (!marker) {
                ...
                marker.style.cssText =
                    'position:absolute;top:-6px;width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-top:8px solid var(--gray-800);transform:translateX(-50%);pointer-events:none;';
    ...
    showCreateListingFolder: async function () {
        ...
        try {
            const folderName = await modals.prompt('Enter a name for the new folder:', {
                title: 'Create Folder',
    ...
    showInventoryAgeAnalysis: function () {
        ...
        const avgAge =
            totalItems > 0 ? Math.round(itemsWithAge.reduce((sum, i) => sum + i.ageInDays, 0) / totalItems) : 0;
    ...
    previewBulkPriceUpdate: function () {
        const inventory = store.state.inventory || [];
        const selectedIds = store.state.selectedInventoryIds || [];
        const items = selectedIds.length > 0 ? inventory.filter((i) => selectedIds.includes(i.id)) : inventory;
        ...
        if (previewEl) {
            ...
            `,
                    ...
                    .join('') +
                    (items.length > 5 ? `<div class="preview-more">...and ${items.length - 5} more items</div>` : ''),
            );
        }
    },
    ...
    showListingHealth: function () {
        ...
        const avgScore =
            analyzed.length > 0 ? Math.round(analyzed.reduce((s, a) => s + a.score, 0) / analyzed.length) : 0;
                                    ...
                                    <div class="health-listing-item">
                                        ...
                                        <div class="health-listing-info">
                                            ...
                                            <div class="health-listing-issues">
                                                ${a.issues
                                                    .slice(0, 2)
                                                    .map((i) => `<span class="issue-tag">${i}</span>`)
                                                    .join('')}
                                            </div>
                                        </div>
                                    </div>
    ...
    showRestockList: function () {
        ...
        restockSuggestions.forEach((item) => {
            const inStock = inventory
                .filter((i) => (i.title || i.name || '').toLowerCase().includes(item.title.toLowerCase().split(' ')[0]))
                        ...
                        <div class="bundle-items-list" id="bundle-items-list">
                            ${
                                ...
                                `;
                                          ...
                                          .join('')
                                    : `<div class="empty-bundle">Add items to create a bundle</div>`
                            }
                        </div>
                    </div>
    ...
    restoreItem: async function (itemId) {
        if (
            !(await modals.confirm('Restore this item? It will be moved back to your inventory as a draft.', {
                title: 'Restore Item',
                confirmText: 'Restore',
            }))
        )
    ...
    permanentlyDeleteItem: async function (itemId) {
        if (
            !(await modals.confirm(
                'PERMANENTLY DELETE this item?<br><br>This action CANNOT be undone. The item will be removed forever.',
                { title: 'Permanent Delete', confirmText: 'Delete Forever', danger: true },
            ))
        )
            return;
    ...
    filterAutomationCategory: function (category) {
        ...
        try {
            localStorage.setItem('vaultlister_automation_category_filter', category);
        } catch (e) {
            console.warn('Failed to save filter preference:', e);
        }
        if (store.state.currentPage === 'automations') {
            const pageContent = pages.automations();
            document.querySelector('.page-content').innerHTML = sanitizeHTML(pageContent); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
    ...
    saveAutomationConfig: function (automationId) {
        ...
        const conditions = Array.from(conditionRows)
            .map((row) => {
    ...
    refreshAllStaleListings: async function () {
        ...
        try {
            ...
            if (data.success) {
                ...
                toast.success(
                    `Refreshed ${data.refreshed} listings. ${data.skipped > 0 ? `Skipped ${data.skipped}.` : ''} ${data.errors > 0 ? `${data.errors} errors.` : ''}`,
                );
    ...
    delistListing: async function (listingId) {
        try {
            ...
            if (data.listing) {
                // Update the listing in state
                const listings = store.state.listings.map((l) => (l.id === listingId ? { ...l, ...data.listing } : l));
                store.setState({ listings });
    ...
    deleteTemplate: async function (templateId) {
        if (
            !(await modals.confirm('Delete this template?', {
    ...
    saveListingAsTemplate: async function () {
        ...
        const templateName = await modals.prompt('Enter a name for this template:', {
            title: 'Save Template',
            defaultValue: `${brand ? brand + ' ' : ''}${category || 'Custom'} Template`,
        ...
        const platformCheckboxes = form.querySelectorAll(
            '.platform-checkbox:checked, .adv-new-platform-checkbox:checked',
        );
    ...
    previewListingImages: function (input, platform) {
        ...
        files.forEach((file, idx) => {
            ...
            reader.onload = (e) => {
                ...
                img.style.cssText =
                    'width:64px;height:64px;border-radius:6px;overflow:hidden;position:relative;border:2px solid var(--gray-200);';
                // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
                img.innerHTML =
                    sanitizeHTML(`<img src="${e.target.result}" alt="Upload preview" style="width:100%;height:100%;object-fit:cover;">
                ...
                if (progressText)
                    progressText.textContent =
                        loaded === files.length
                            ...
                            : `Processing ${loaded}/${files.length}...`;
            };
    ...
    displayAIResults: function (data) {
        ...
        const confidenceBadge =
            confidence === 'high' ? 'badge-success' : confidence === 'medium' ? 'badge-warning' : 'badge-gray';
    ...
    displayIdentifyResults: function (data) {
        ...
        const confidenceClass =
            confidenceVal >= 80 ? 'badge-success' : confidenceVal >= 50 ? 'badge-warning' : 'badge-gray';
    ...
    submitAdvancedCrosslist: async function (event, itemIds) {
        ...
        try {
            ...
            if (workflowMode === 'unified') {
                ...
                for (const itemId of ids) {
                    for (const platform of selectedPlatforms) {
                        ...
                        const tags = formData.get(`${platform}Tags`)
                            ? formData
                                  .get(`${platform}Tags`)
                                  .split(',')
                                  .map((t) => t.trim())
            ...
            if (typeof gtag === 'function')
                gtag('event', 'cross_list', {
                    platforms_count: selectedPlatforms.length,
                    listings_count: listings.length,
                    listing_platform: selectedPlatforms.join(','),
                });
            ...
            if (error.message && error.message.includes('CSRF')) {
                try {
                    ...
                    toast.success(
                        `Created ${listings.length} listings across ${selectedPlatforms.length} platform(s)!`,
                    );
                    modals.close();
                    await handlers.loadListings();
                    store.setState({ crosslistSelectedItems: [] });
    ...
    deleteSkuRule: async function (ruleId) {
        if (
            !(await modals.confirm('Are you sure you want to delete this SKU rule?', {
                title: 'Delete SKU Rule',
                confirmText: 'Delete',
                danger: true,
            }))
        )
    ...
    batchUpdateSkus: async function (ruleId, onlyEmpty = true) {
        ...
        if (
            !(await modals.confirm(
                `Apply rule "${rule?.name || 'Unknown'}" to ${onlyEmpty ? 'items without SKUs' : 'ALL items'}? This cannot be undone.`,
                { title: 'Apply SKU Rule', confirmText: 'Apply', danger: true },
            ))
        ) {
            return;
        }
    ...
    previewSkuPatternForRule: async function (ruleId, pattern) {
        try {
            ...
            if (previewEl && previewSku) {
                previewEl.innerHTML = sanitizeHTML(
                    `<code class="bg-primary-100 text-primary-700 px-2 py-1 rounded">${escapeHtml(previewSku)}</code>`,
                ); // nosemgrep: javascript.browser.security.insecure-document-method.insecure-document-method
            }
        } catch (error) {
            console.error('Preview error:', error);
    ...
    deleteRelistingRule: async function (ruleId) {
        if (
            !(await modals.confirm('Delete this relisting rule?', {
                title: 'Delete Rule',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    toggleAllStale: function (checked) {
        document.querySelectorAll('.stale-checkbox').forEach((cb) => {
            cb.checked = checked;
        });
    },
    ...
    cancelImport: function () {
        ...
        if (jobId) {
            api.post(`/inventory-import/jobs/${jobId}/cancel`, {}).catch((err) =>
    ...
    startColResize: function (e, colIndex) {
        ...
        guide.style.cssText =
            ...
            'px;height:' +
            wrapRect.height +
            'px;width:2px;background:var(--primary-500);z-index:9999;pointer-events:none;';
    ...
    startRowResize: function (e, rowIndex) {
        ...
        guide.style.cssText =
            ...
            'px;width:' +
            wrapRect.width +
            'px;height:2px;background:var(--primary-500);z-index:9999;pointer-events:none;';
    ...
    validateImport: async function () {
        ...
        if (filterCheckboxes.length > 0) {
            filterCheckboxes.forEach((cb) => {
                if (cb.checked) visibleCols.add(parseInt(cb.dataset.filterCol));
            });
        } else {
        ...
        const vlFields = [
            {
                value: 'title',
                aliases: ['title', 'name', 'item_name', 'product_name', 'product_title', 'listing_title', 'itemname'],
            },
            ...
            {
                value: 'quantity',
                aliases: ['quantity', 'qty', 'on_hand', 'inventory_count', 'qtyonhand', 'qty_on_hand'],
            },
        ...
        if (!fieldMapping.title) {
            toast.error(
                'No "Title" column detected. Ensure your CSV has a column named Title, Name, Item Name, or Product.',
            );
            return;
        }
    ...
    saveMappingTemplate: async function () {
        const name = await modals.prompt('Enter a name for this mapping template:', {
            title: 'Save Template',
            placeholder: 'Template name',
        });
        if (!name) return;
    ...
    deleteImportJob: async function (jobId) {
        if (
            !(await modals.confirm('Delete this import job?', {
                title: 'Delete Job',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    deleteMapping: async function (mappingId) {
        if (
            !(await modals.confirm('Delete this mapping template?', {
                title: 'Delete Template',
                confirmText: 'Delete',
                danger: true,
            }))
    ...
    runBulkOptimize: async function () {
        ...
        toast.success(
            `Optimization complete! ${optimized} improvements suggested across ${Math.min(listings.length, 20)} listings.`,
        );
        router.navigate('listings');
    },
    ...
    showItemOfferHistory: function () {
        ...
        modals.show(
            ...
            <div class="modal-body item-offer-history">
                ...
                ${
                    items.length === 0
                        ? '<p class="text-center text-gray-500 py-8">No offer history yet</p>'
                        : items
                              .map((item, idx) => {
                                  const accepted = item.offers.filter((o) => o.status === 'accepted').length;
                                  const declined = item.offers.filter(
                                      (o) => o.status === 'declined' || o.status === 'rejected',
                                  ).length;
                                  const pending = item.offers.filter((o) => o.status === 'pending').length;
                                  const avgOffer =
                                      item.offers.reduce((s, o) => s + (o.amount || 0), 0) / item.offers.length;
                                  const avgPct =
                                      item.listing_price > 0 ? Math.round((avgOffer / item.listing_price) * 100) : 0;
                                ...
                                ${item.offers
                                    ...
                                    .map((o) => {
                                        ...
                                        const statusClass =
                                            o.status === 'accepted'
                                                ? 'success'
                                                : o.status === 'declined' || o.status === 'rejected'
                                                  ? 'error'
                                                  : 'warning';
                                        return (
                                            '<div class="offer-timeline-entry">' +
                                            '<div class="offer-timeline-date">' +
                                            date +
                                            '</div>' +
                                            '<div class="offer-timeline-amount">$' +
    ...
    setAcquiredDate: async function (itemId) {
        const date = await modals.prompt('Select the date this item was acquired:', {
            title: 'Set Acquired Date',
            inputType: 'date',
        });
        if (!date) return;
        try {
    ...
    async permanentlyDelete(id) {
        if (
            !(await modals.confirm('Permanently delete this item? This cannot be undone.', {
    ...
    async bulkPermanentlyDelete() {
        ...
        if (
            !(await modals.confirm(`Permanently delete ${ids.length} item(s)? This cannot be undone.`, {
                title: 'Permanent Delete',
                confirmText: 'Delete Forever',
                danger: true,
            }))
    ...
    async cleanupOldDeleted() {
        if (
            !(await modals.confirm('Permanently delete all items deleted more than 30 days ago?', {
    ...
    onFilterColumnChange: function (column) {
        ...
        if (column === 'status') {
            ...
            sel.innerHTML =
                '<option value="">All</option><option value="active">Active</option><option value="draft">Draft</option><option value="low_stock">Low Stock</option><option value="out_of_stock">Out of Stock</option><option value="stale">Stale</option>';
