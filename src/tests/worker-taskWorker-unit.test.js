...
describe('taskWorker (unit)', () => {
  ...
  // =================================================================
  describe('queueTask', () => {
    test('returns a task object with correct fields', async () => {
      const result = await queueTask('sync_shop', { shopId: 'shop-1', userId: 'user-1' });
    ...
    test('calls query.run with INSERT statement and correct values', async () => {
      const payload = { shopId: 'shop-1', userId: 'user-1' };
      await queueTask('sync_shop', payload);
    ...
    test('uses default priority of 0', async () => {
    ...
    test('accepts custom priority option', async () => {
      const result = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' }, { priority: 10 });
      expect(result.priority).toBe(10);
      const params = mockQueryRun.mock.calls[0][1];
      expect(params[3]).toBe(10);
    });

    test('accepts custom maxAttempts option', async () => {
      const result = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' }, { maxAttempts: 5 });
      expect(result.maxAttempts).toBe(5);
      const params = mockQueryRun.mock.calls[0][1];
      expect(params[4]).toBe(5);
    });

    test('accepts custom scheduledAt option', async () => {
      const scheduledAt = '2026-03-01 12:00:00';
      const result = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' }, { scheduledAt });
      expect(result.scheduledAt).toBe(scheduledAt);
      const params = mockQueryRun.mock.calls[0][1];
      expect(params[5]).toBe(scheduledAt);
    });

    test('generates a non-empty string ID (real UUID)', async () => {
      const result = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' });
      expect(typeof result.id).toBe('string');
      expect(result.id.length).toBeGreaterThan(0);
    });

    test('serializes payload to JSON in the INSERT', async () => {
    ...
    test('returns the payload object (not serialized)', async () => {
      const payload = { shopId: 'shop-99', userId: 'user-42' };
      const result = await queueTask('sync_shop', payload);
      expect(result.payload).toEqual(payload);
    });

    test('works with different task types', async () => {
      ...
      for (const type of types) {
        ...
        const result = await queueTask(type, { userId: 'u1' });
        expect(result.type).toBe(type);
        expect(mockQueryRun).toHaveBeenCalledTimes(1);
      }
    });

    test('scheduledAt defaults to a PostgreSQL-compatible datetime format (no T or Z)', async () => {
      const result = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' });
    ...
    test('multiple queueTask calls generate unique IDs', async () => {
      const r1 = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' });
      const r2 = await queueTask('sync_shop', { shopId: 's2', userId: 'u2' });
  ...
  describe('getTaskStatus', () => {
    test('returns the task record from the database', async () => {
      const taskRecord = {
        id: 'task-abc',
        type: 'sync_shop',
        status: 'completed',
      ...
      const result = await getTaskStatus('task-abc');
      expect(result).toEqual(taskRecord);
    });

    test('calls query.get with the correct SQL and task ID', async () => {
      mockQueryGet.mockReturnValue(null);
      await getTaskStatus('task-xyz');
    ...
    test('returns undefined when query.get returns undefined', async () => {
      mockQueryGet.mockReturnValue(undefined);
      const result = await getTaskStatus('no-task');
      expect(result).toBeUndefined();
    });

    test('returns a pending task', async () => {
      const pendingTask = {
      ...
      const result = await getTaskStatus('pending-1');
      expect(result.status).toBe('pending');
      expect(result.started_at).toBeNull();
    });

    test('returns a failed task with error info', async () => {
      const failedTask = {
      ...
      const result = await getTaskStatus('failed-1');
  ...
  describe('getWorkerStatus', () => {
    test('returns expected shape with all properties', async () => {
    ...
    test('maxConcurrent is 3', async () => {
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      const status = await getWorkerStatus();
      expect(status.maxConcurrent).toBe(3);
    });

    test('pollIntervalMs is 10000 (10 seconds)', async () => {
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      const status = await getWorkerStatus();
      expect(status.pollIntervalMs).toBe(10000);
    });

    test('isRunning is false when worker is stopped', async () => {
      stopTaskWorker();
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      const status = await getWorkerStatus();
    ...
    test('last24Hours contains the stats from the database', async () => {
      const dbStats = {
    ...
    test('activeTasks is a number', async () => {
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      const status = await getWorkerStatus();
      expect(typeof status.activeTasks).toBe('number');
    });

    test('query uses 24 hour window', async () => {
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
    ...
    test('query counts tasks by status', async () => {
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      await getWorkerStatus();
      const sql = mockQueryGet.mock.calls[0][0];
  ...
  describe('cleanupOldTasks', () => {
    test('returns the number of deleted tasks', async () => {
      mockQueryRun.mockReturnValue({ changes: 15 });
      const result = await cleanupOldTasks(7);
      expect(result).toBe(15);
    });

    test('defaults to 7 days when no argument is provided', async () => {
    ...
    test('passes custom daysOld to the query', async () => {
    ...
    test('deletes only completed and failed tasks', async () => {
      mockQueryRun.mockReturnValue({ changes: 0 });
    ...
    test('uses DELETE FROM task_queue', async () => {
      mockQueryRun.mockReturnValue({ changes: 0 });
      await cleanupOldTasks(14);
      const sql = mockQueryRun.mock.calls[0][0];
      expect(sql).toContain('DELETE FROM task_queue');
    });

    test('returns 0 when no tasks are old enough', async () => {
      mockQueryRun.mockReturnValue({ changes: 0 });
      const result = await cleanupOldTasks(365);
      expect(result).toBe(0);
    });

    test('uses completed_at for age comparison', async () => {
      mockQueryRun.mockReturnValue({ changes: 0 });
      await cleanupOldTasks(7);
      const sql = mockQueryRun.mock.calls[0][0];
      expect(sql).toContain('completed_at');
    ...
    test('with daysOld of 1 works correctly', async () => {
      mockQueryRun.mockReturnValue({ changes: 100 });
      const result = await cleanupOldTasks(1);
  ...
  describe('startTaskWorker / stopTaskWorker', () => {
    ...
    test('startTaskWorker starts the worker (isRunning becomes true)', async () => {
      stopTaskWorker();
      startTaskWorker();
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      const status = await getWorkerStatus();
    ...
    test('stopTaskWorker stops the worker (isRunning becomes false)', async () => {
      startTaskWorker();
      stopTaskWorker();
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      const status = await getWorkerStatus();
    ...
    test('calling startTaskWorker twice does not create duplicate intervals', async () => {
      stopTaskWorker();
      startTaskWorker();
    ...
    test('start then stop then start cycle works correctly', async () => {
      stopTaskWorker();
      startTaskWorker();
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      expect((await getWorkerStatus()).isRunning).toBe(true);
      stopTaskWorker();
      expect((await getWorkerStatus()).isRunning).toBe(false);
      startTaskWorker();
      expect((await getWorkerStatus()).isRunning).toBe(true);
      stopTaskWorker();
    });
  ...
  // =================================================================
  describe('edge cases', () => {
    test('queueTask with empty payload object', async () => {
      const result = await queueTask('cleanup_notifications', {});
      expect(result.payload).toEqual({});
      const params = mockQueryRun.mock.calls[0][1];
      expect(params[2]).toBe('{}');
    });

    test('queueTask with nested payload data', async () => {
      const payload = { userId: 'u1', config: { deep: { nested: true } }, tags: ['a', 'b'] };
      const result = await queueTask('run_automation', payload);
      expect(result.payload).toEqual(payload);
      const params = mockQueryRun.mock.calls[0][1];
      expect(JSON.parse(params[2])).toEqual(payload);
    });

    test('queueTask with priority 0 is stored correctly', async () => {
      const result = await queueTask('process_webhook', { eventId: 'e1' }, { priority: 0 });
      expect(result.priority).toBe(0);
      const params = mockQueryRun.mock.calls[0][1];
      expect(params[3]).toBe(0);
    });

    test('queueTask with negative priority is stored', async () => {
      const result = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' }, { priority: -5 });
      expect(result.priority).toBe(-5);
      const params = mockQueryRun.mock.calls[0][1];
      expect(params[3]).toBe(-5);
    });

    test('queueTask with maxAttempts of 1 means single try only', async () => {
      const result = await queueTask('refresh_token', { shopId: 's1', userId: 'u1' }, { maxAttempts: 1 });
      expect(result.maxAttempts).toBe(1);
    });

    test('queueTask ID is a valid UUID v4 format', async () => {
      const result = await queueTask('sync_shop', { shopId: 's1', userId: 'u1' });
      // Real UUID v4: 8-4-4-4-12 hex pattern
      expect(result.id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
    });

    test('queueTask INSERT params order matches VALUES placeholders', async () => {
      const payload = { shopId: 's1', userId: 'u1' };
      await queueTask('sync_shop', payload, { priority: 5, maxAttempts: 2, scheduledAt: '2026-06-01 00:00:00' });
      const params = mockQueryRun.mock.calls[0][1];
    ...
    test('getWorkerStatus activeTasks starts at 0', async () => {
      stopTaskWorker();
      mockQueryGet.mockReturnValue({ total_tasks: 0 });
      const status = await getWorkerStatus();
    ...
    test('cleanupOldTasks with large daysOld still works', async () => {
      mockQueryRun.mockReturnValue({ changes: 0 });
      const result = await cleanupOldTasks(9999);
      expect(result).toBe(0);
      const params = mockQueryRun.mock.calls[0][1];
